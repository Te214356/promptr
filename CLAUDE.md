...
3. **ربط Google AdSense** — ✅ **تدقيق ما قبل التقديم اكتمل (2026-08-06)**، ولم يبقَ سوى إنشاء الحساب:
   ...
   **حالة الكود (2026-08-06):** الحساب أُنشئ ومعرّف الناشر `ca-pub-1113985459345993`. مكتبة AdSense **مُحمَّلة على مستوى الموقع** عبر `modules/common/components/adsense-script` المُركَّب في `app/[countryCode]/(main)/layout.tsx` — والمعرّف في ثابت واحد مُصدَّر هناك.

   > ⚠️ **لماذا لم تبقَ محصورة في `/blog`:** جرّبنا ذلك أولًا فرفضت Google إثبات الملكية («تعذّر إثبات ملكية موقعك») — زاحف التحقق يفحص **الصفحة الرئيسية**، ولم يكن السكربت فيها. النطاق الآن كامل عدا استثناءين رخيصين: مجموعة `(checkout)` لها layout مستقل فلا ترث المكوّن أصلًا، و`/account` يُستثنى بفحص `usePathname` داخل المكوّن.
   ...
   > 🔎 **ملاحظة تقنية على شكل الوسم:** `next/script` بـ`afterInteractive` لا يضع وسم `<script>` في HTML الصادر من الخادم — يضع `<link rel="preload" … as="script">` ويحقن السكربت بعد الترطيب (hydration). رابط الـpreload يحوي معرّف الناشر كاملًا، فأي فحص يبحث عن المعرّف نصيًا سيجده. **لكن إن رفضت Google التحقق مجددًا**، فالسبب الأرجح أنها تبحث عن وسم `<script>` فعلي في HTML الخام، والحل حينها نقل السكربت إلى `app/layout.tsx` بـ`strategy="beforeInteractive"` — بثمن معلوم: الـroot layout يغلّف كل شيء، فيسقط استثناء `/account` و`(checkout)`.
