...
**8 مقالات منشورة من هدف 15–20** (انظر *الخطوة التالية* أدناه).
...
**الربط الداخلي:** كل مقال يحيل لمقال أو مقالين ذوي صلة (بمسار `/blog/<slug>`) إضافةً لمنتجه. عند إضافة مقال جديد، اربطه من مقال قائم أيضًا — لا تتركه معزولًا. تحقّق أن كل slug داخلي يطابق ملفًا موجودًا فعلًا في `content/blog/`.
...
3. **ربط Google AdSense** — البند 2 أُنجز، لكن التقديم ما زال يحتاج **عددًا كافيًا من المقالات الأصلية المنشورة**؛ حاليًا 8 مقالات من هدف 15–20. موضع الإعلانات محجوز مسبقًا بتعليق في `apps/storefront/src/app/[countryCode]/(main)/blog/layout.tsx` — الإعلانات في `/blog` فقط، لا في صفحات المتجر أو الدفع.

---

## الخطوة التالية: بناء أرشيف المقالات

**الهدف:** 15–20 مقالًا أصليًا منشورًا — وهو ما يحتاجه تقديم AdSense (البند 3 أعلاه). المنشور اليوم: **8 مقالات** (الجدول في قسم *المدونة*).
