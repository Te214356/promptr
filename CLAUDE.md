...
**خمسة كوميتات محلية، ولا شيء منها على `main` ولا على `origin`.** توقّف العمل عند قرار لم يُحسم، لا عند عطل.
...
### ⚠️ معلّق (1) — البند الأخير من `code-review`: `cart_id` بلا تحقق ملكية

`moyasar-callback` يقبل أي `cart_id` من الرابط ثم يُشغّل `initiatePaymentSession` و`completeCart` عليه. **سابق لهذا الفرع، غير محسوم، ولم يُرقَّع.** التفاصيل وما يحتويه اليوم (تحقق الباك إند من الدفعة + مقارنة المبلغ والعملة) موثّقة عند الكود نفسه وفي قسم *صفحة الدفع*. **مرشَّح لقائمة ما قبل تفعيل Moyasar الحي.**
