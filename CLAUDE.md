...
3. **ربط Google AdSense** — ✅ **تدقيق ما قبل التقديم اكتمل (2026-08-06)**، ولم يبقَ سوى إنشاء الحساب:
   ...
   **حالة الكود (2026-08-06):** الحساب أُنشئ ومعرّف الناشر `ca-pub-1113985459345993`. في `app/layout.tsx` (الـroot) **وسمان معًا**، والمعرّف في ثابت واحد يغذّيهما:
   ...
   | 3 | `<meta name="google-adsense-account">` عبر `metadata.other` | ❌ رفض **أيضًا** رغم تأكيد الوسم في `<head>` على الإنتاج |

   **الوسمان الثلاثة (ميتا + سكربت + `ads.txt`) كلها مؤكَّدة حيًّا بـ`curl`، والرفض مستمر** — أي أن المشكلة على الأرجح ليست في محتوى الصفحة بل في **وصول زاحف AdSense إليها**. الفرضية قيد التشخيص (2026-08-06): حجب أو تحدٍّ أمني أمام `Mediapartners-Google` / `AdsBot-Google`. لا تُضف وسمًا رابعًا قبل حسم هذه الفرضية.

   **الخلاصة العملية حتى الآن:** لأي تحقق ملكية مستقبلي — **ابدأ بالعلامة الوصفية عبر `metadata.other`** لا بالسكربت (أرخص ولا يعتمد على تنفيذ جافاسكربت)، **ثم تحقق فورًا أن الزاحف نفسه يصل للصفحة** قبل افتراض أن المشكلة في الوسم.
