...
**11 مقالًا منشورًا من هدف 15–20** (انظر *الخطوة التالية* أدناه).
...
**المنتجات المستخدمة كمصدر حتى الآن:** `ecommerce-success-guide` (أقسام 2/3/5/6)، `customer-service-prompts` (2/5)، `ai-basics-arabic` (2)، `ecommerce-prompts-arabic` (1)، `cv-guide-graduates`، `midjourney-arabic-prompts`. **لم تُستخدم بعد:** `chatgpt-arabic-prompts`، `chatgpt-prompts-pro-arabic`، `marketing-prompts-arabic`، `social-media-templates`، `ai-video-guide-arabic`، `ai-income-book`، `digital-marketing-saudi-guide`.
...
3. **ربط Google AdSense** — البند 2 أُنجز، لكن التقديم ما زال يحتاج **عددًا كافيًا من المقالات الأصلية المنشورة**؛ حاليًا 11 مقالًا من هدف 15–20 — أي 4–9 مقالات متبقية. موضع الإعلانات محجوز مسبقًا بتعليق في `apps/storefront/src/app/[countryCode]/(main)/blog/layout.tsx` — الإعلانات في `/blog` فقط، لا في صفحات المتجر أو الدفع.

---

## الخطوة التالية: بناء أرشيف المقالات

**الهدف:** 15–20 مقالًا أصليًا منشورًا — وهو ما يحتاجه تقديم AdSense (البند 3 أعلاه). المنشور اليوم: **11 مقالًا** (الجدول في قسم *المدونة*).
