...
## ✅ مُغلق — فرع `fix/moyasar-payment-errors` دُمج ونُشر (2026-09-07)

> **الفرع مدموج في `main` ومنشور على الإنتاج، والحارسان حيّان هناك.** ما تحت هذا العنوان تاريخ العمل ولماذا اتُّخذ كل قرار — يُقرأ للفهم، **لا كقائمة مهام**. أي عبارة فيه بصيغة «لم يُكوَّت» أو «لم يُنشر» أو «ما تبقّى» فقد انقضت.

### 🎯 حالة الإغلاق

| البند | الحالة |
|---|---|
| **الشرط [أ] — شراء كامل من طرف إلى طرف** | ✅ **مستوفى** (2026-09-07). طلب حقيقي بـ49 ريالًا عبر متصفح: ميسر `paid` ⟵ `metadata.cart_id` مقروءًا من الخادم بالمفتاح السري يطابق السلة ⟵ صف مطالبة بمعرّفين متطابقين ⟵ طلب مكتمل و`payment.captured_at` مضبوط ⟵ روابط التحميل تعمل |
| **الدمج** | ✅ `main` عند `bab4cfd` (تقديم سريع، تسعة كوميتات) |
| **نشر الستورفرنت** | ✅ `d554c462` — الستورفرنت **أولًا** كما يقتضي الترتيب أدناه |
| **نشر الباك إند** | ✅ `03dd2429` من `bab4cfd`؛ الهجرة نجحت وأنشأت `moyasar_payment_claim` على الإنتاج بالفهرس غير الجزئي |
| **الحارس على الإنتاج** | ✅ 409 بالرسالة العربية · صفر رفض حتى الآن · الجدول فارغ |
| **`RESEND_API_KEY` على الإنتاج** | ✅ سليم — مفتاح **مقيّد بالإرسال فقط** (`restricted_api_key`)، فردّه 401 على `/domains` سلوك صحيح لا عطل. **لا تستنتج من ذلك أنه ميت** |
| **`/code-review` (لا ultra)** | ✅ شُغّل؛ عولجت الثلاثة ذات الأثر المالي، وأُجّلت اثنتان منخفضتان (أدناه) |
...
2. **بندان منخفضان من مراجعة الكود، مؤجَّلان بقرار:**
   ...
   - `cart_unavailable` **غير قابل للوصول**: صفحة السلة تبتلع فشل الباك إند بـ`.catch(() => null)` فتعرض **سلة فارغة** أثناء الانقطاع — أي الرسالة الوحيدة التي وُجد الإشعار ليمنعها. تفريق 404 عن انقطاع النقل صار رخيصًا بعد تقسيم `retrieveCart` في هذا الفرع.

3. **`/code-review ultra` لم يُشغَّل** — شُغّلت المراجعة العادية فقط لضيق الوقت. يُطلقها المستخدم من الطرفية؛ لا يستطيع Claude إطلاقها.

4. **البريد لم يُختبر على الإنتاج بعد** — المفتاح سليم، لكن لم يُرسل بريد اختبار بقرار المستخدم؛ يتحقق منه بشراء حقيقي. (محليًا فشل الإرسال بـ401 لأن المفتاح المحلي ميت — 24 محرفًا مقابل 36 على الإنتاج. **عطل بيئة محلية لا كود:** توليد الروابط نجح والفشل التُقط وسُجّل بلا كسر الطلب.)

---

### 🧭 تاريخ العمل — ما قيس ولماذا (أُبقي للفهم لا للتنفيذ)
...
**📦 حالة المستودع وقت كتابة هذا القسم (2026-09-05) — انقضت: دُمج ونُشر في 2026-09-07، انظر كتلة الإغلاق أعلى الملف.**

- ~~**ستة كوميتات محلية** على `fix/moyasar-payment-errors`، ولا شيء منها على `main` ولا على `origin`.~~ ⟵ **انقضى:** تسعة كوميتات، مدموجة ومنشورة (2026-09-07).
- ~~**وعمل الحارسين كله لم يُكوَّت بعد**~~ ⟵ **كُوِّت في `3c8a133`** — تعديلات غير مرحّلة في `moyasar/service.ts` · `moyasar-form/index.tsx` · `moyasar-callback/page.tsx` · `cart/templates/index.tsx` · `medusa-config.ts` · `railway.json`، وملفات جديدة: `api/middlewares.ts` · `modules/payment-guard/` · مجلّدا `__tests__` · `integration-tests/`.
- **`/code-review ultra` لم يُشغَّل** (شُغّلت المراجعة العادية بدلًا منها). يُطلقه المستخدم من الطرفية؛ لا يستطيع Claude إطلاقه.
- **ترتيب ما تبقّى:** ~~المفاتيح~~ ✅ ⟵ ~~الشرط [أ]~~ ✅ ⟵ ~~المراجعة~~ ✅ (عادية لا ultra) ⟵ ~~الدمج~~ ✅ ⟵ ~~النشر~~ ✅ **كله منجز 2026-09-07**.


**ستة كوميتات محلية وقتها؛ صارت تسعة ودُمجت في `main` عند `bab4cfd` (2026-09-07).** الجدول أدناه سجل لما في كل كوميت.
...
>
> ⛔ **انقضى العذر (2026-09-07): الدفع الحي مفعَّل.** كان مكتوبًا هنا أن هذا «محتمَل لا مقبول» لأن الدفع الحي غير مفعَّل فلا يوجد جمهور دفعات يُعطب. **`MOYASAR_SECRET_KEY` على الإنتاج يبدأ بـ`sk_live_`، والقابل للنشر `pk_live_` في الطرفين.**
>
> فترتيب النشر أدناه صار **الفاصل الوحيد** بين نشرة متعجّلة ومشترين مخصومين بلا طلبات — لا تحوّطًا نظريًا. وعلاج من يُخصم منه ثم تُرفض دفعته **استرداد يدوي، لا إعادة محاولة** (إعادة المحاولة خصم ثانٍ).
>
> ✅ **ولم يقع ضرر في نشرة 2026-09-07:** نُشر الستورفرنت أولًا كما تقتضي القاعدة، وفُحص حساب ميسر الحي بعدها فكانت فيه **6 دفعات فقط، كلها من 28 أغسطس وكلها نهائية** (3 `refunded` + 3 `failed`) — **صفر دفعة في حالة `paid` أو `authorized` بلا `metadata.cart_id`**، أي صفر عميل دفع بلا طلب.
>
...
> ⛔ **لكن التفعيل تمّ قبل إغلاقها** — فهي لم تعد بوابات تسبق التفعيل بل **ديونًا متأخرة على نظام يقبض فعلًا**. انظر رأس القائمة.
...
### 📦 `.railwayignore` — بدونه يفشل الرفع بمهلة (أُنشئ 2026-09-07)

الحمولة كانت **60MB**، منها 32MB `product-images-backup/` و23MB `products/` — لا يقرأ البناء أيًّا منها. فانتهى `railway up` بـ`operation timed out`، **وترك نشرة عالقة في `INITIALIZING` بلا بناء مرتبط** ثم فشلت. الملف يقلّصها إلى **~3MB**.

> ⛔ **كل نمط مجلّد يجب أن يبدأ بشرطة `/`.** الملف يتبع دلالات gitignore، فنمط `products/` بلا شرطة يطابق **أي** مجلد بهذا الاسم على أي عمق — واستثنى فعلًا `apps/storefront/src/modules/products/` و`app/[countryCode]/(main)/products/`، فسقط البناء على `Can't resolve '@modules/products/components/thumbnail'`.
>
> ⛔ **ولا تستثنِ `apps/backend`:** مساحات عمل npm (`apps/**`) تحلّ كل ملفات `package.json` وقت التثبيت، فحذفه يكسر التثبيت وإن كان المبني هو الستورفرنت وحده.
>
> **التحقق قبل أي رفع — بمحرّك git لا بمطابقة يدوية:**
> ```bash
> SP=$(mktemp -d); git init -q "$SP"; cp .railwayignore "$SP/.gitignore"
> git ls-files -co --exclude-standard -z \
>   | git -C "$SP" check-ignore --no-index --stdin -z | tr '\0' '\n' | grep '^apps/'
> ```
> **يجب أن يعود فارغًا.** (`git check-ignore` **لا يقبل** `--exclude-from` — تمريره يُخرج خطأً، ولو كُتم بـ`2>/dev/null` لأظهر «صفر مستثنى» كاذبًا. هذا ما أخفى العطل أول مرة.)
...
~~**Remaining external blocker:** Moyasar live-payment account activation is pending (support ticket `94773`).~~ ✅ **مغلق — الحساب مفعَّل.** أُنجزت به عمليات شراء حقيقية (حسب المالك). مقيس 2026-09-07: `MOYASAR_SECRET_KEY` على الإنتاج `sk_live_`، والقابل للنشر `pk_live_` ومتطابق بين الباك إند والستورفرنت.
> قراءة حساب ميسر الحي في نفس اليوم: **6 دفعات، كلها 28 أغسطس ونهائية** — 3 `refunded` (1.00 · 2.45 · 2.45 ريال) و3 `failed` بـ49 ريالًا. لا دفعة معلّقة.
...
**الإغلاق الصحيح** ربط مُعرّف السلة بجلسة المشتري **قبل** مغادرته إلى ميسر — وهو نفس الربط الذي كان ينقص التوقيع المحذوف. **تغيير مستقل، ولا يُرقَّع بسطر هنا.** ✅ **نُفِّذ ونُشر (2026-09-07)** — انظر كتلة الإغلاق في رأس الملف.
...
### ~~قبل تفعيل Moyasar الحي~~ — ديون متأخرة على نظام يقبض فعلًا

> ⛔ **أُعيد تأطير هذه القائمة (2026-09-07).** كُتبت كبوابات **تسبق** التفعيل — **والتفعيل تمّ ولم تُغلق كلها.** فلا تُقرأ بعد اليوم كشرط مستقبلي: البنود المفتوحة (3 · 4 · 5) هي نواقص أمنية **على متجر يقبل الدفع الحي الآن**، ووزنها ارتفع لا انخفض.

ما كان يجب إتمامه قبل التفعيل:
