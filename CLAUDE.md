...
**14 مقالًا منشورًا من هدف 15–20** (انظر *الخطوة التالية* أدناه).
...
**المتبقي صالحًا كمصدر:** `chatgpt-arabic-prompts` (8 أقسام) و`chatgpt-prompts-pro-arabic` (5 أقسام) — وهما يكفيان لبقية الهدف.
...
3. **ربط Google AdSense** — البند 2 أُنجز، لكن التقديم ما زال يحتاج **عددًا كافيًا من المقالات الأصلية المنشورة**؛ حاليًا 14 مقالًا من هدف 15–20 — أي 1–6 مقالات متبقية. موضع الإعلانات محجوز مسبقًا بتعليق في `apps/storefront/src/app/[countryCode]/(main)/blog/layout.tsx` — الإعلانات في `/blog` فقط، لا في صفحات المتجر أو الدفع.

---

## الخطوة التالية: بناء أرشيف المقالات

**الهدف:** 15–20 مقالًا أصليًا منشورًا — وهو ما يحتاجه تقديم AdSense (البند 3 أعلاه). المنشور اليوم: **14 مقالًا** (الجدول في قسم *المدونة*).
