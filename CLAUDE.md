...
3. **ربط Google AdSense** — البند 2 أُنجز، لكن التقديم ما زال يحتاج **عددًا كافيًا من المقالات الأصلية المنشورة**؛ حاليًا **17 مقالًا** — تجاوز الحد الأدنى (15)، فلم يعد العدد مانعًا للتقديم. موضع الإعلانات محجوز مسبقًا بتعليق في `apps/storefront/src/app/[countryCode]/(main)/blog/layout.tsx` — الإعلانات في `/blog` فقط، لا في صفحات المتجر أو الدفع.
