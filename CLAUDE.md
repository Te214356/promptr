...
### ✍️ رسائل الكوميت العربية: استعمل `-F -` لا `-m "$(printf …)"`

```bash
git commit -F - <<'MSG'      ✅ الصيغة الوحيدة المأمونة
…
MSG
```

> ⛔ **`git commit -m "$(printf '…')"` يكسر النص العربي.** الصدفة تفسّر ما بداخل `$( )` قبل أن يصل إلى git: الأقواس الهلالية في «(المعلومات العامة)» تُقرأ كتجميع أوامر، والعلامة `'` في كلمة إنجليزية مثل `owner's` تفتح اقتباسًا، و`\n` يصل حرفيًا لا سطرًا جديدًا.
>
> **وقع هذا فعلًا في 2026-09-13 (كوميت `96220b1`):** خرجت الرسالة مشوّهة — سطر الموضوع جزءٌ من منتصف النص و`\n` ظاهرة حرفيًا. **والمحتوى كان سليمًا**، فتُرك الكوميت كما هو: إصلاح الرسالة كان يقتضي `--force-push` على `main`، وهي مخاطرة حقيقية مقابل تحسين تجميلي. **القرار: لا يُعاد كتابة `main` لأجل رسالة.**
...
## 📌 نقطة الاستئناف — 2026-09-15 (اقرأها أولًا)

> # 🎯 الأولوية القادمة **ليست تقنية**: جلب أول زوار حقيقيين.
>
> **لم يبقَ بند تقني يمنع بيعًا أو يضرّ مشتريًا، والواجهة اكتملت بقرار المالك (2026-09-15).** الدفع يعمل، والحارسان يحرسان، والتسليم يصل، والنصوص تُقرأ، وشارات الثقة حقيقية، والسيو نظيف، واسم المتجر يظهر للمشتري صحيحًا، والرئيسية تعرض منتجات حقيقية بخط عربي محمَّل. فلا تبدأ الجلسة القادمة ببند تقني إلا إن طلبه المالك صراحةً.
>
> وحجم الفجوة: **17 مقالًا · 5 نقرات من بحث Google في 28 يومًا · صفر بيع لعميل**. البندان الوحيدان في القائمة اللذان يخدمان هذه الأولوية هما **`هـ1` AdSense** و**`هـ2` مقالات جديدة**. الأدوات في `.claude/agents/` (بحث · كتابة · يوتيوب · استشارة).
>
> **ما أُغلق في 2026-09-14/15 (عشرة كوميتات `47b3750`…`1f750a7`):** المرور البصري كاملًا — التفاصيل والقياسات في قسم *🎨 مُغلق — المرور البصري* أدناه. **لا تُعِد فحص** الخط ولا الرفّ ولا البطاقات ولا البنرين ولا رأس الجوال.

### ✅ مُغلق نهائيًا — **لا يُعاد فحصه**

| المجال | الدليل |
|---|---|
| **مسار الشراء كاملًا** | شراء بشري حقيقي: الدفع ⟵ الحارسان ⟵ الطلب 25 ⟵ البريد ⟵ رابط التحميل، من ثلاثة مصادر متطابقة |
| **حارسا الدفع** | مقيسان على شراء حقيقي لا على تجربة |
| **webhook ميسر** | محصَّن بسرّ مشترك (**لا HMAC**) ومعطَّل بقرار؛ ثلاث حمولات مزوّرة رُفضت على الإنتاج |
| **تباين WCAG AA** | 139 راسبًا ⟵ 16، وكلها زر واتساب المستثنى |
| **`SameSite` وآلية الاسترجاع** | الجدول مقيس في WebKit؛ الحذف كان صحيحًا. المقياس محفوظ: `scripts/samesite-probe.mjs` |
| **السيو** | canonical على كل صفحة مفهرسة · soft-404 صار 404 · التصنيفات في الخريطة (40 رابطًا) · الروابط الداخلية بلا تحويلات · `next-sitemap` المحذوف |
| **كاش الكتالوج** | `revalidate: 60` — مقيس بتعديل سعر حقيقي: 59 ثانية ذهابًا و11 إيابًا |
| **TLS على `api.promptrsa.com`** | Let's Encrypt · 200 في 0.46s بعد 80 يومًا عطلًا |
| **الهوية وميسر** | الشعار مرفوع · `public_name = Promptr` مؤكَّدًا من API · `og:image` لكل مقال |
| **Cloudflare R2** | `promptr-files` ⟵ `Public Access: Disabled` |
| **خلفية `#080810`** | ثلاثة بدائل قيست: **صفر مكسب**. بقيت بقرار — **وأُعيد تأكيده في المرور البصري 2026-09-15**: كل الأسطح الجديدة بُنيت عليها وعلى `#0d0d1f` |
| **الواجهة (المرور البصري)** | Cairo على الموقع كله · الرئيسية بمنتجات حقيقية (رفّ + شريط 6 + بطاقات بأغلفة وعدد) · صفر سطح فاتح · رأس الجوال بفراغات مقيسة. LCP على الحي: 1544 ⟵ 1324–1468ms |

### ⏳ المفتوح — عشرة بنود، ولا شيء منها عاجل

**أمن:** 1. `أ3` مراجعة أمنية بشرية · 2. `أ4` CSP (يُبدأ بـ`Report-Only`)

**أثر على البيع:** 3. الإبطال الفوري لكاش الكتالوج (**ابدأ بالوسم الثابت لا بالـsubscriber**) · 4. تفعيل webhook ميسر (**محجوب بـ`session_id` المفقود لا بالتوقيع**) · 5. فشل داخل نموذج MPF (**يحتاج دفعة حقيقية مرفوضة**) · 6. `ب5` `requires_shipping: false` صريحًا · `ب2` عمرا الكوكيين

**واجهة:** 7. صفحة 404 عارية · 8. `د8` الجزيرتان الفاتحتان (**لم يمسّهما المرور البصري** — حقول التواصل ونموذج ميسر ما زالا فاتحين) · `د9` واتساب (مستثنى) · `د6` `app/error.tsx` الجذري · `د7` اليتيمان (`PaymentButton` وحده الآن؛ `featured-products` عاد للاستعمال) · `د10` `sku` · `د11` **هوامش الشعار على الجوال** لا تُخصَم من الأصفار إن كُبِّر الشعار: القاعدة في قسم رأس الجوال

**ديون:** 9. `د3` `/code-review ultra` (يُطلقه المالك) · `د4` ترقيات npm · `د5` عدّادات الذاكرة ⟵ Redis

**محتوى:** 10. **`هـ1` AdSense** (معلومات الدفع · صفر وحدة إعلانية) · **`هـ2` مقالات جديدة**

---

## 🎨 مُغلق — المرور البصري على الواجهة (2026-09-14/15)

**بدأ بتشخيص لا بتعديل:** لقطات كاملة من الإنتاج بمحرك WebKit (1440 و390) للرئيسية والمتجر وصفحة المنتج، ثم أربعة معايير مقيسة لا انطباع. **الخلاصة التي وجّهت كل ما بعدها:** الرئيسية كانت **2624px بلا منتج واحد**، وأقوى أصل بصري في الموقع (12 غلافًا بهوية موحّدة) لا يظهر إلا لمن نقر «استكشف المتجر». والمتجر **لم يكن يحمّل أي خط عربي** — صفر `@font-face` — فكل نص عربي يُرسم بخط نظام الجهاز.

**عشرة كوميتات على `main`، كلها منشورة ومقيسة على الحي بعد النشر:**

| الكوميت | ما فيه |
|---|---|
| `47b3750` | Cairo على الموقع كله · زر «أضف للسلة» بلون الهوية · «ذات صلة» بلغة واحدة و4 منتجات · شريط الترتيب بلا نقطة |
| `f95e79b` | صندوق سطر أوسع لنص البطل المتدرّج (نوازل Cairo كانت تُقصّ) |
| `4429bcc` | **الرفّ الأول:** بطل بنصفين + شريط منتجات + حذف البنر المكرِّر + `sizes` مطابقة للشبكات + شرطة الترتيب على جهة البداية |
| `d1226c1` | بطاقات التصنيفات بأغلفة منتجاتها وعددها من Store API · **`lib/data/home.ts` يوزّع الأغلفة على الأقسام بلا تكرار** |
| `0ae0cc0` · `f7032b3` | البنران على أرضية البطاقات الداكنة · الزخرفة على جهة النهاية المنطقية |
| `2c21d1b` · `7563378` · `ed8cb02` | زخرفتان تدلّان على الموضوع (مسار طلبات · شبكة عصبية) · مخفيتان تحت 1024 · مكبَّرتان إلى 70–80% |
| `1f750a7` | رأس الجوال: فراغات حقيقية · الشعار 17px تحت 512 · السلة أيقونة بشارة |

### القياسات قبل ⟵ بعد (كلها على `promptrsa.com` بمحرك WebKit إلا ما ذُكر)

| البند | قبل | بعد |
|---|---|---|
| `@font-face` على المتجر | **0** — H1 يعلن `Segoe UI` والجسم `Inter` غير المحمَّل | Cairo 200–1000 `loaded`؛ H1 والجسم و`txt-*` كلها Cairo |
| صور في الشاشة الأولى للرئيسية (1440) | **0** · منتجات في الصفحة كلها **0** | **7** صور (3 أغلفة + 4 بطاقات) · **6** منتجات بأسعارها + 5 أغلفة في البطاقات |
| ارتفاع البطل | 810px (`min-h-[90vh]`) · H1 72px ممركز | 584px · H1 56px على جهة البداية · الشريط يبدأ داخل الشاشة الأولى |
| الفوتر من طول الصفحة | 32% (838 من 2624) | 23% (838 من 3636) |
| زر «أضف للسلة» | `rgb(82,82,91)` · 40px — الزر الوحيد بلا لون الهوية | `rgb(108,43,255)` · 48px · في الصفحة والشريط اللاصق |
| «منتجات ذات صلة» | عنوان ثنائي اللغة · 11 منتجًا | «قد يعجبك أيضًا» · 4 |
| بطاقات التصنيفات | 220px · أيقونة 28px في الزاوية · بلا عدد | 420px · عنوان 24px · أغلفة حقيقية (2·2·1) · «8 منتجات · 3 منتجات · منتج واحد» من Store API |
| تكرار الأغلفة بين الشريط والبطاقات | 3 (حتمي: 8 + 7 = 15 خانة من 12 منتجًا) | **0** — شريط 6 + بطاقات 5 = 11 من 12 |
| البنران | السطحان الفاتحان الوحيدان في صفحة داكنة · زخرفة هندسية عامة | `#0d0d1f` بغسلة من لون التمييز · رسم يدلّ على الموضوع بحبر 78–79% من الشريحة وفراغ 185/162px حتى النص (1440) |
| رأس الجوال: الشعار ⟵ المبدّل | **0px** على 390 و430 · «السلة (0)» على **3 أسطر** | 46px / 66px · السلة سطر واحد (أيقونة + شارة) |
| LCP (Chrome، ~4Mbps + 4× CPU، وسيط 5) | 1544ms (H1) | **1324–1468ms** (غلاف الرفّ) — أول دفعة بعد كل نشر أعلى (كاش محسِّن الصور بارد) |
| tsc | 248 | 245 · صفر جديد |

### القرارات وأسبابها — لا تُعاد مناقشتها بلا معطى جديد

**1. الخلفية بقيت `#080810`.** قيست ثلاثة بدائل قبل هذا المرور بصفر مكسب، وكل الأسطح الجديدة (البطاقات والبنران والرفّ) بُنيت على `#080810` و`#0d0d1f`. المشكلة كانت **الفراغ لا اللون** — كما شخّصها المالك قبل القياس.

**2. أصناف `txt-*` من ui-preset تكتب `Inter` حرفيًا** لا من `theme.fontFamily` (مقيس من CSS المبني). فنقل Cairo إلى الجذر وحده لا يكفي: عناوين المنتجات والأسعار وخيارات الترتيب كانت ستبقى على خط النظام. القاعدة `[class^="txt-"],[class*=" txt-"]{font-family:inherit}` في `globals.css` **ضرورية وتُبقى**، وتحقّقنا أنها تأتي بعد قاعدة الـpreset في الملف المبني.

**3. نوازل Cairo أعمق من خط النظام** و`bg-clip-text` لا يرسم إلا داخل صندوق السطر. أي نص متدرّج جديد يحتاج `leading` أوسع أو `pb`، وإلا اختفى ذيل «في» و«ي» كما وقع على الإنتاج بين `47b3750` و`f95e79b`.

**4. الشريط 6 لا 8، والبطاقات غلافان لا ثلاثة — حساب لا ذوق.** الكتالوج 12 منتجًا؛ 8 + (3+3+1) = 15 خانة ⟵ 3 تكرارات حتمية مهما اختير. التوزيع في `lib/data/home.ts` مكان واحد يقرّر الأقسام الثلاثة: البطاقات أولًا (2·2·1)، ثم الشريط مما بقي، ثم رفّ البطل من المتبقي وآخر الشريط (صفه الثاني، تحت الشاشة الأولى). **حين يتجاوز الكتالوج 13 منتجًا** يمكن رفع `RAIL_COUNT` إلى 8 و`COVERS_PER_CARD` إلى 3 — ثابتان، ولا يُغيَّران تلقائيًا.

**5. عدد المنتجات في البطاقات من `/store/products?category_id=…` لا من توسيع `*products`** على التصنيفات — نفس فخ الملاحظة 7 في قائمة الفحص: التوسيع يعطي 8/6/3 بينما الزائر يرى 8/3/1.

**6. زخرفة البنر مخفية تحت 1024px — قرار صحيح بقرار المالك.** على 820px النص يمتد من 140 إلى 716 من 772 (140px حرّة)، وعلى 390 يأخذ العرض كله (صفر). رسم «مصغّر» في 140px شبكة بارتفاع ~70px تُقرأ كبقعة، وتحت النص مرفوض. البديل الوحيد تضييق العنوان على 820 إلى 30px — **رُفض: لا نضحّي بحجم العنوان لأجل زخرفة.** فوق 1024 الحجم نسبي من عرض الشريحة (44% ثم 48% من 1280) والنص مقيَّد (42% ثم 40%) فيثبت الفراغ عبر عروض سطح المكتب.

**7. السلة أيقونة بشارة تحت 512px، ونص «السلة (n)» فوقها.** النص كان ينكسر على ثلاثة أسطر على 390. `aria-label` يحمل النص في الحالتين، والشارة مقيسة على الحي بإضافة منتج فعلي: «1» على `rgb(108,43,255)` 16×16، ولا تُرسم على سطح المكتب (`display:none`).

**8. رأس الجوال: العمودان الجانبيان `basis-auto` تحت 512 لا `basis-0`.** القسمة بالتساوي تجعل العمود الأعرض (المبدّل + السلة) يزحف نحو الشعار (5px على 360 بينما الجهة الأخرى 57). بـ`basis-auto` يتوزّع الفراغ الحرّ بالتساوي (30/31 على 360 · 45/46 على 390 · 65/66 على 430) — ثمنه انزياح الشعار بضعة بكسلات عن المنتصف الهندسي على الجوال، وهو مقبول. **والشعار 17px بتباعد 0.18em تحت 512** (كان 22px / 0.25em ثابتًا؛ عرضه 159 ⟵ 115). أي تكبير له على الجوال يُقاس بـ`scripts` القياس أدناه لا بالنظر.

**9. الرفّ في البطل يشارك الشريط عمدًا** — عيّنة منه، محصورة في صفه الثاني. ليس تكرارًا يُصلَح.

**10. الصورة الأمامية في الرفّ تحمل `priority`** (مرشّح LCP)، والبقية لا. و`ProductPreview` يمرّر `sizes` مطابقة لشبكاته (2/3/4 أعمدة) — كان يطلب 800px لخانة 330px؛ الأثر يشمل المتجر و«ذات صلة».

### ما لم يُمَس ويبقى مفتوحًا

الجزيرتان الفاتحتان (حقول التواصل ونموذج ميسر) · صفحة 404 · زر واتساب الأخضر (أعلى إشباعًا في كل شاشة، مستثنى بقرار) · صورة وثيقة العمل الحر في الفوتر تحمل `priority` وهي أسفل الصفحة (14KB — صغيرة، لم تُلمس) · «السلة (0)» على الجوال كانت تنكسر أيضًا في القالب الأصلي منذ الإطلاق.

### أدوات القياس المحفوظة في مجلد الجلسة (يُعاد إنشاؤها بسهولة)

- **لقطات كاملة + قياسات**: `playwright-core` + WebKit (`~/Library/Caches/ms-playwright/webkit-2359`، يطابق `playwright-core@1.63`) — `npm i playwright-core` في مجلد مؤقت ثم `webkit.launch()`.
- **LCP**: Chrome عبر `chromium.launch({ channel: "chrome" })` مع `Network.emulateNetworkConditions` و`Emulation.setCPUThrottlingRate(4)` و`PerformanceObserver('largest-contentful-paint')`. **WebKit لا يكشف LCP.** اقرأ الوسيط من 5 مرات، **وتجاهل أول دفعة بعد النشر** (محسِّن الصور بارد: 1760 ثم 1324).
- **تداخل الزخرفة**: `getBBox()` للـSVG محوَّلًا إلى بكسل الشاشة مقابل `getBoundingClientRect()` للعنوان والوصف والزر — **حبر الرسم لا صندوقه**.
- **رأس الجوال**: فراغات `left − right` بين القائمة والشعار والمبدّل والسلة على 360/390/430/768/1440، وعدد سطور السلة من `height / lineHeight`.
- **معاينة للمالك**: صفحة HTML بصور **مضمّنة `data:` URI** (JPEG عبر `sips`). الروابط إلى ملفات مرفقة **لا تفتح عنده**، و`SendUserFile` يصل كمسار محلي. الـdata URI هو ما نجح ثلاث مرات.

---

## 🎨 مُغلق — تباين النص: كتلة الوضع الداكن كانت ميتة على الإنتاج (2026-09-09)

**نُشر على `main` عند `1bec35c` وبناء Railway `SUCCESS`.** ما تحت هذا العنوان قواعد دائمة وبند مفتوح واحد — لا قائمة مهام.

### السبب الجذري — محدِّد واحد لا يطابق

| الطرف | ما كان |
|---|---|
| `apps/storefront/tailwind.config.js` | `darkMode: "class"` ⟵ فيكتب ui-preset اللوحة الداكنة تحت **`.dark`** |
| `apps/storefront/src/app/layout.tsx:26` | `<html … data-mode="dark">` ⟵ **سمة، لا صنف** |

فلم تُطابق كتلة `.dark` شيئًا قط، **وكل رمز `text-ui-fg-*` في المتجر كان يُرسم بقيمته الفاتحة فوق `#080810`**. مقيس من CSS الإنتاج لا من الكود: `--fg-base` كان له تعريفان اثنان فقط (`:root` و`.dark`)، وكلمة `data-mode` **صفر مرة** في الملف. وبتأكيد مستقل عبر `getComputedStyle` على الصفحة الحيّة: العنوان والسعر `rgb(24,24,27)` على `rgb(8,8,16)`.

**ما كلّفه ذلك على الصفحة التي تقبض المال:**

| العنصر | قبل | بعد |
|---|---|---|
| **الإجمالي** (`cart-totals:75`) | **1.13:1** | 18.15:1 |
| **سعر المنتج** · عنوان المنتج | **1.13:1** | 18.15:1 |
| المجموع الفرعي/الضريبة · سطر الموافقة | 2.58:1 | 7.78:1 |
| الحدود والفواصل | 1.18–1.32:1 | 3.76–3.83:1 |

`darkMode: ["class", '[data-mode="dark"]']` — والمحدِّد المخصص يمرّره ui-preset حرفيًا إلى الكتلة التي يولّدها.

> ### ⛔ القاعدة (1): أي رمز أمامي جديد يُقاس على `#080810` من CSS **المبني**
>
> لا يُفترض من نية الكود ولا من اسم الرمز. القياس العملي:
> ```bash
> npm run build --workspace=@dtc/storefront
> grep -o '\[data-mode=dark\]{[^}]*}' apps/storefront/.next/static/css/*.css   # القيمة الفعّالة
> ```
> ثم احسب النسبة على `#080810` (الصفحة) و`#0d0d1f` (بطاقة السلة). **وحتى لوحة Medusa الداكنة لا تكفي بذاتها:** `--fg-muted` الداكن `#71717A` يعطي 4.13:1 و3.97:1 — **يرسب على السطحين**، فرُفع إلى `#8E8E97` (6.14 / 5.91) في `globals.css`. لا تُعِده إلى قيمة Medusa ظنًّا أنها «الأصل».

> ### ⛔ القاعدة (2): `darkMode` و`globals.css` (تثبيت الأسطح) يتغيّران معًا — كوميت واحد
>
> كتلة الوضع الداكن تقلب **الأسطح** أيضًا: `--bg-field` من `#FAFAFA` إلى `rgba(255,255,255,.04)`. وسطر `input, select, textarea { color:#000 }` يفرض نصًّا أسود على الحقول. فالمحدِّد وحده يجعل الحقل داكنًا والنص أسود عليه ⟵ **1.13:1، أي العطل نفسه منقولًا من النصوص إلى الحقول**.
>
> لذلك تُثبَّت الأسطح المستعملة (`--bg-field` · `--bg-field-hover` · `--bg-subtle` · `--bg-base` · `--bg-disabled` · `--bg-component-pressed` · `--border-base`) على قيم `:root` في `globals.css`. **لا يُحذف أحد الطرفين وحده.**
>
> ونتيجةً لذلك، الرموز الأمامية القليلة التي تقع **فوق** تلك الأسطح الفاتحة مثبَّتة بقيم حرفية لا برموز: `common/components/input` · `native-select` · `cart-item-select` · `common/components/modal` · `onboarding-cta` (مجلّدان). أي `text-ui-fg-*` يُضاف فوق سطح فاتح سيصير غير مقروء.

**ونطاق الانقلاب أوسع مما يبدو — قيس ولا تفترض:** **81 رمزًا** يشير إليه صنف مولَّد تغيّرت قيمته الفعلية (أزرار · وسوم · ظلال · حلقات تركيز)، لأنها كلها كانت تُرسم من اللوحة الفاتحة. فُحص كل زوج نص/خلفية بينها: **لا شيء يهبط تحت AA**؛ وأكبر حركة **الزر الأساسي 11.82:1 ⟵ 6.43:1** بخلفية أفتح (قُبلت بقرار المالك: تعبر AA بفارق مريح وتخدم كنداء للفعل).

### 🕳️ الفخ وقع فعلًا — في حقلين داكنين لا في الجزيرتين (2026-09-09)

حذّرت القاعدة (2) من اقتران `input{color:#000}` بالأسطح، ثم **أخطأتُ موضع الفخ**: بحثت عنه في الجزيرتين الفاتحتين، وكان قائمًا **منذ ما قبل هذا العمل** في حقلين خلفيتهما `bg-white/5` داكنة:

| الحقل | ما كان |
|---|---|
| `discount-code/index.tsx:80` — كود الخصم | يعلن `text-white`، ويُرسم **`rgb(0,0,0)`** ⟵ **1.15:1** |
| `cart/components/item/index.tsx:76` — قائمة الكمية | نفس الشيء ⟵ **1.21:1** |

**السبب:** الكلمة `!important`. صنف `text-white` محدِّده `(0,1,0)` ويغلب محدِّد العنصر `(0,0,1)` — **لكنه لا يغلب `!important`**. فحُذفت الكلمة وبقي السطر أساسًا عاملًا.

> ⛔ **لا تُعِد `!important` إلى هذا السطر.** وجوده ضروري (Tailwind preflight يضبط `color: inherit` على عناصر النماذج، فترث الأبيض من `body` وتصير بيضاء على أبيض)، لكن `!important` يجعله يبتلع **كل** صنف لون — فأي حقل داكن جديد يُكسر صامتًا، ولا يظهر العطل إلا لمن يكتب في الحقل.
>
> **والفحص الذي كان سيمسكه من أول مرة، وأغفلته:** قِس **لون النص المُدخَل** لا النائب وحده. النائب كان `placeholder-white/60` عند 7.11:1 بينما النص المكتوب 1.15:1 — عنصران مختلفان في نفس الحقل، وقياس أحدهما لا يقول شيئًا عن الآخر.
>
> ```js
> // على الصفحة الحيّة، لكل حقل في مسار الشراء
> getComputedStyle(document.querySelector('#promotion-input')).color
> ```

### 🚫 ولا تعرض نص خطأ خامًا من Server Action

في الإنتاج **يستبدل Next رسالة أي خطأ يُرمى داخل Server Action** بالنص الإنجليزي `An error occurred in the Server Components render...`. وكان `applyPromotions` يرمي و`DiscountCode` يطبع `e.message` — فوصلت تلك الجملة إلى المشتري عند كل فشل.

**اليوم:** `applyPromotions` **يُرجع نتيجة ولا يرمي** (`PromotionResult` في `lib/data/cart.ts`)، و`DiscountCode` يُسمّي سببًا عربيًا واحدًا لكل حالة.

> ⚠️ **و«الكود غير صالح» ليس خطأً من ميدوسا:** `updateCartPromotions` يحسب الإجراءات للأكواد التي يعرفها **ويُسقط الباقي بصمت** (الأكواد المجهولة بلا إجراء، وتجاوز الحد يقع في `skippedPromoCodes` ولا يُرجعه مسار الـStore). **فالـ200 لا يثبت شيئًا** — الإشارة الوحيدة الموثوقة هي **قراءة النتيجة**: `promotions.code` من السلة العائدة، وكل كود لم يهبط = مرفوض.
>
> ⚠️ **وافصل الانقطاع عن الكود الخاطئ** — بالحالة (`FetchError.status`) لا بالرسالة: `4xx` ⟵ الكود، وما عداه ⟵ الخادم. قول «كودك خاطئ» أثناء انقطاع يُرسل المشتري يبحث عن كود آخر وكوده سليم. نفس درس ترتيب أخطاء الدفع.

### 🔄 التحقق البصري من CSS منشور يلزمه **تحديث قسري** — والتصفح الخاص لا يكفي

**أضاعت هذه المسألةُ ثلاث جولات كاملة (2026-09-09/10):** أُصلح عطل لون النص في الحقول ونُشر، وظل المالك يراه أسود في **نافذة تصفح خاص جديدة** في سفاري، فبدا الإصلاح فاشلًا ثلاث مرات وهو ناجح. الحسم جاء بـ**`Cmd+Option+R`** (تحديث قسري): النص أبيض.

**السبب:** نافذة التصفح الخاص لا تبدأ بكاش فارغ بالمعنى الذي يُفترض — **تحتفظ بكاش داخل الجلسة نفسها**، ولا يُفرَّغ إلا بإغلاق النافذة كليًا. ففتح تبويب خاص جديد **ليس** فحصًا نظيفًا.

> ### القاعدة
> **لا تحكم على إصلاح CSS منشور من نافذة تصفح خاص.** استعمل **تحديثًا قسريًا** (`Cmd+Option+R` في سفاري، `Cmd+Shift+R` في كروم)، أو افحص من محرك آلي بملف تعريف جديد.
>
> **وللوكيل:** الفحص الحاسم هو قراءة **بصمة ملف CSS** التي تحمّلها الصفحة فعلًا ومقارنتها ببصمة البناء المنشور:
> ```js
> [...document.styleSheets].map(s => s.href).filter(Boolean)
> ```
> بصمة مختلفة = كاش، لا عطل. **اسأل عن البصمة قبل أن تُعيد فتح ملف مُصلَح.**

### 🔬 وقياس الأنماط يكون على **العنصر الحقيقي في متصفح حقيقي** — لا على عنصر تحقنه بنفسك

في نفس الجولات، قِست حقل كود الخصم بحقن `<input>` يحمل قائمة الأصناف **كما أظنها**، فقرأت أبيض وأعلنت النجاح — بينما العطل قائم. **التجربة كانت تُثبت أن CSS يوافق افتراضي، لا أن الصفحة سليمة** (نفس قاعدة «افحص من أين تأتي مدخلات التجربة»).

**ومقياس جاهز محفوظ في المستودع:** `apps/storefront/scripts/samesite-probe.mjs` — يحسم سلوك كوكي السلة ومسار «إعادة المحاولة» على الموقع الحي.
...
> ⛔ **أعِد تشغيله عند أي تغيير يمسّ** خصائص كوكي `_medusa_cart_id` · `moyasar-callback` · `checkout` · أو `middleware.ts`.

**الأداة الصحيحة موجودة الآن ومجرَّبة:** WebKit عبر `playwright-core` — **نفس محرك سفاري**، يُقاد آليًا على الموقع الحي: يضيف منتجًا للسلة بالنقر، يفتح صندوق الخصم، **يكتب نصًّا فعليًا**، ثم يقرأ `getComputedStyle` من العنصر نفسه ويسرد القواعد المتطابقة عليه.

```bash
npx playwright@latest install webkit     # مرة واحدة
# ثم playwright-core: webkit.launch() ⟵ page.fill() ⟵ page.evaluate(getComputedStyle)
```

> ⛔ **حقل فارغ لا يُقاس، والنائب ليس النص المُدخَل.** اكتب فيه ثم اقرأ.
> ⛔ **وانتبه لتركيب الشفافيات عند حساب الخلفية:** طبقات `rgba` متداخلة تُركَّب من الأسفل للأعلى، **والخلفية المتدرّجة (`background-image`) لا تُقاس أصلًا** — لا تُبلّغ عنها كرسوب. أخطأتُ في الاثنين فأنتج فاحصي ثلاثة صفوف زائفة (عنوان الرئيسية «منتجات رقمية جاهزة» على تدرّج بنفسجي فاتح ظهر بنسبة 1.00:1 وهو سليم تمامًا).

### ⏳ بند مفتوح — الجزيرتان الفاتحتان

**حقول بيانات التواصل** (`bg-ui-bg-field` = `#FAFAFA` بنص `#000`) و**نموذج ميسر** (داخل `bg-white` في `moyasar-form/index.tsx:266`) لوحان فاتحان داخل واجهة داكنة.

> **يعبران القراءة بامتياز** (18.9:1 و15.9:1) — **فمشكلتهما هوية لا تباين**، وهي مؤجَّلة بقرار لمرور منفصل. ولذلك استُثنيا من هذا الكوميت، وأُبقي حد صندوق ميسر على `white/10` وحده دون بقية الحدود.
>
> ⚠️ **وأي معالجة هوية لهما تبدأ بحذف تثبيت الأسطح أعلاه — ووقتها يجب أن يتغيّر سطر `input{color:#000}` في نفس الكوميت.** القاعدة (2) تنطبق على المسار العكسي أيضًا.

### 📣 والدرس الأثقل: الإشارة سبقت القياس بشهور

**أبلغ المالك عن ضعف التباين مرارًا منذ إنشاء المتجر** *(كما ذكر؛ لا سجل له في هذا الملف قبل اليوم — وهذا جزء من العطل لا هامش عليه)*، وظل البند يُعامَل كتحسين تجميلي مؤجَّل حتى قِيس فوُجد **1.13:1 على الرقم الذي يُخصم من المشتري**.

> **القاعدة:** بلاغ متكرر من المالك عن شيء **يراه على الشاشة** يُقاس فورًا، ولا يُصنَّف «تجميليًا» قبل القياس. تكلفة القياس هنا كانت دقائق؛ وتكلفة التأجيل متجرٌ يعرض مبالغه بأسود على أسود على مدى شهور، **وثلاث محاولات شراء حقيقية فاشلة في 28 أغسطس لم يُعرف سببها** (انظر قسم ميسر). لا يوجد دليل يربط الاثنين — ولا يوجد ما ينفيه.
...
### ✅✅ الشرط [أ] مُغلق نهائيًا — شراء بشري حقيقي على الإنتاج (2026-09-11)

**مسار الشراء مؤكَّد من طرف إلى طرف على `promptrsa.com`، بمفاتيح حيّة وبيد المالك:** الدفع ⟵ الحارسان ⟵ الطلب ⟵ البريد ⟵ رابط التحميل. الطلب **25** بريال واحد (بخصم `TEST-JSB4…` fixed 4800 على منتج بـ49).

**والسلسلة مقيسة من ثلاثة مصادر مستقلة تتطابق، لا من سطر لوق واحد:**

| المصدر | ما يقوله |
|---|---|
| **سجل ميسر** (قراءة خادمية بالمفتاح السري) | `ecb5a34b-…` · `paid` · **100 SAR** · `visa creditcard` · **`metadata.cart_id = cart_01M2716P818XX03J9NMRJTRFV6`** |
| **`moyasar_payment_claim`** | صف **واحد في الجدول كله**: نفس `moyasar_id` ونفس `cart_id` · `deleted_at = NULL` |
| **`payment`** | نفس `moyasar_id` · `data.cart_id` نفسه · `amount = 100` **يساوي** `expected_amount = 100` · `captured_at` مضبوط |

**وترتيب الزمن يثبت التسلسل المصمَّم:** الدفعة أُنشئت `01:34:38`، والمطالبة كُتبت `01:34:44.59`، والطلب `01:34:44.71` — أي **المطالبة قبل الطلب بـ0.12 ثانية**، وهو بالضبط ما يقتضيه وقوعها على `complete`.

> ✅ **فالفرضيتان الحاملتان للحارسين صارتا مقيستين على شراء حقيقي لا على تجربة:** ميسر **خزّن `metadata.cart_id` وأعاده**، والاشتقاق الخادمي طابقه؛ والفهرس الفريد قَبِل مطالبة واحدة ولم يُكرّرها.
>
> ✅ **والتسليم مقيس أيضًا:** البريد وصل من `orders@promptrsa.com` والرابط الموقَّع فتح الملف (أبلغ المالك) — وهو ما كان البند 4 أدناه ينتظره.
>
> ⛔ **ولا يُستدَل بهذا على أن المتجر «باع»:** المشتري هو المالك، والدفعة استُردّ منها لا شيء لأنها رمزية. الأرقام في قسم حساب ميسر.

**والكود الاختباري عُطِّل فور الإغلاق** (`status: inactive`)، وقرأناه قبل التعطيل عند `used: 1 / limit 1` — أي أن حدّ المرة الواحدة عمل فعلًا على شراء حقيقي.
...
~~4. **البريد لم يُختبر على الإنتاج بعد**~~ — ✅ **أُغلق 2026-09-11 بشراء بشري حقيقي:** البريد خرج من `orders@promptrsa.com`، ورابط التحميل الموقَّع فتح الملف. انظر قسم الإغلاق من طرف إلى طرف. *(الملاحظة المحلية تبقى للفهم: فشل الإرسال محليًا بـ401 لأن المفتاح المحلي ميت — 24 محرفًا مقابل 36 على الإنتاج؛ عطل بيئة لا كود، والفشل التُقط وسُجّل بلا كسر الطلب.)*
...
### ✅ مُغلق — آلية الاسترجاع **لم تكن مطلوبة** (قيس 2026-09-12)
...
**وقد قيس الشقّ الثاني أيضًا (2026-09-12) فحُسم لصالح الحذف.** التسلسل نُفِّذ كاملًا في WebKit على الموقع الحي، بوصول **cross-site حقيقي** من أصل آخر كما تفعل ميسر:

```
وصول cross-site إلى الـcallback بـstatus=failed
   ⟵ «فشل الدفع» · زر «إعادة المحاولة» حاضر
   ⟵ وسجل الستورفرنت يؤكد: [moyasar-callback] payment failed
نقر «إعادة المحاولة»
   ⟵ /sa/checkout?step=payment  ·  ✅ خطوة الدفع بالسلة سليمة
```

> ✅ **فالحذف في `6dac231` صحيح بالقياس لا بالحجة.** و`Strict` يحجب الإرسال ولا يحذف الكوكي، فالنقرة الداخلية تُعيده — وهو ما كان الجدول أعلاه يقوله استنتاجًا وصار مقيسًا.
>
> ⛔ **فلا تُعِد بناء أي آلية استرجاع** لهذا المسار. وإن ظهر يومًا سبب جديد، **ابدأ بإعادة تشغيل `samesite-probe.mjs`** — إن بقيت نتيجته كما هي فالسبب في مكان آخر.

### ⏳ بند مفتوح منفصل — الفشل **داخل نموذج MPF** غير مغطّى

ما قيس أعلاه هو فشل يعود عبر الـcallback (3DS، أو إلغاء على صفحة البنك). **أما بطاقة مرفوضة بلا 3DS فيعالجها MPF في الصفحة نفسها بلا أي تنقّل** — فلا callback ولا زر «إعادة المحاولة»، والنموذج يبقى بمحتواه وبيانات البطاقة المكتوبة.

> ⛔ **ولا يُقاس بحمولة مصاغة:** لا يوجد رابط يُصاغ، والمسار كله داخل نموذج ميسر. **يحتاج دفعة حقيقية مرفوضة** (بطاقة مدى `4201321234411220`). مؤجَّل بقرار، ولم يُنفَّذ.

### تفاصيل الحذف — تُقرأ للفهم
...
## 📌 ~~نقطة استئناف — آخر تحديث 2026-08-08~~ (انقضت — انظر نقطة الاستئناف في رأس الملف)
...
~~1. **البند 3** — تعطيل `Public Development URL` لـ`promptr-files`~~ ✅ **مُغلق 2026-09-11** — كان معطَّلًا أصلًا؛ فُحص بصريًا ولم يحتج تدخلًا.
...
~~**Remaining external blocker:** Moyasar live-payment account activation is pending (support ticket `94773`).~~ ✅ **مغلق — الحساب مفعَّل** منذ 2026-08-26.

### 🏦 حساب ميسر الحي — أي كيان، وماذا فيه فعلًا (مقيس 2026-09-07)

**المفتاح الحي في Railway يخصّ هذا الكيان — لا تخلطه بغيره:**

```
entity_id      = 42b3e83a-b22b-4336-880e-0c4daf535d30   ⟵ الحساب الفعّال
account_id     = bc314109-e49f-4eee-a310-22734994877a
account_type   = live · activated_at 2026-08-26 · deactivated_at = null
```

مقروء من `GET /v1/account` بالمفتاح السري نفسه، وهو **يطابق الكيان الذي يسمّيه إيميل ميسر فعّالًا**.

> ⚠️ **و`f9cb58bd-79b6-432f-a248-8289d48f9906` كيان آخر لا يحمل مفاتيحنا.** ظهر في مراسلات ميسر فأربك التشخيص جولة كاملة. **لا تبحث عن مبيعات مفقودة فيه:** مفاتيح ميسر مقيّدة بالكيان، فما يرجعه `/v1/payments` بمفتاحنا هو **كامل تاريخ `42b3e83a`** لا عيّنة منه — ولا يستطيع هذا المفتاح قراءة أي كيان آخر أصلًا.
> و`entity_id` غير `account_id`: حقلان مختلفان في نفس الاستجابة، وخلطهما يولّد تناقضًا وهميًا.

**والمتجر لم يبع لعميل حقيقي بعد** — ولم يتغيّر هذا في 2026-09-11: أُضيفت **دفعة رابعة مقبوضة بريال واحد**، لكنها **شراء المالك الاختباري** لا بيعًا (طلب 25، بخصم `TEST-…` استُهلك مرة واحدة). رسم ميسر عليها **1.18 ريال** — أي أكثر من قيمتها، وهو طبيعي في مبلغ رمزي. وما دون ذلك كما كان حتى 2026-09-07: الحساب الحي يحمل **6 دفعات، كلها 28 أغسطس وكلها نهائية**:

| الحالة | العدد | المبالغ | ما هي |
|---|---|---|---|
| `refunded` | 3 | 1.00 · 2.45 · 2.45 ريال | **اختبارات المالك بمال حقيقي، استُردّت كلها.** ليست مبيعات |
| `failed` | 3 | 49 ريالًا ×3 | **محاولات شراء حقيقية فشلت** — انظر أدناه |

تطابقها في قاعدة الإنتاج: **ثلاث دفعات مقبوضة فقط** (100 · 245 · 245 هللة) على الطلبات **22 و23 و24** — اثنان بـ`tareq_ksa1@hotmail.com` وواحد «Test Product» بـ`troog20@gmail.com`. والطلبات 1–21 من يوليو بلا دفعة مقبوضة (عهد مفاتيح الاختبار).

> 💡 والرصيد يؤكدها: `total: -697` أي **−6.97 ريال** — رسوم الاستردادات الثلاثة وحدها. حساب لم يبِع بعد.
>
> ⛔ **فلا تستشهد بـ«ثلاث مبيعات» كدليل على أن مسار الدفع يعمل لعميل.** المسار لم يُختبر بعد إلا بيد المالك.

### 🔴 ثلاث محاولات شراء حقيقية فشلت — 28 أغسطس، 49 ريالًا لكلٍّ

`failed` ×3 بقيمة 49 ريالًا في نفس اليوم، من زوّار لا من المالك (اختباراته كانت بريال وريالين ونصف). **أقرب ما لدينا إلى عملاء حقيقيين، وكلهم خرجوا بلا شراء.**

**السبب لم يُقَس** — لم تُحفظ رسالة الفشل ولا سجلّ الخادم من ذلك اليوم. لكن الأعطال التي أُصلحت في فرع `fix/moyasar-payment-errors` **مرشّحة مباشرة**، وكلها كانت حيّة في 28 أغسطس:

- **الزر الأحمر الصامت** — يفشل الدفع بلا أي رسالة تُقرأ (`8960a5e`)
- **404 على صفحة الدفع** عند غياب كوكي السلة (`8cd517f`)
- **فروع ما بعد الخصم** التي كانت تعرض «إعادة المحاولة» (`126f0d0` · `6dac231`)
- **تصنيف الأخطاء** الذي كان يسمّي انقطاع الشبكة «رفض بطاقة» فيُرسل المشتري يجرّب بطاقات بلا جدوى

> 🔎 **ما يحسمها:** أول فشل جديد بعد نشر 2026-09-07 صار يُسجَّل بـ`[moyasar-callback] payment failed` مع الرمز المصنَّف والرسالة الخام. **راقب هذا السطر** — هو أول مرة نملك فيها أثرًا لفشل حقيقي بدل تخمينه بأثر رجعي.
...
## 🎨 أصول الهوية وحساب ميسر العلني (2026-09-12)

### الملفات — `apps/storefront/public/images/brand/`

| الملف | متى يُستعمل |
|---|---|
| `logo-512.png` · `logo-1024.png` | **مفرّغ**: تدرّج داخل الحروف على شفاف — لأسطح الموقع الداكنة |
| `logo-filled-512.png` · `logo-filled-1024.png` | **ممتلئ**: مربّع متدرّج و«P» أبيض — **للخلفيات البيضاء** (فواتير ميسر) |
| `og-default.png` | بطاقة مشاركة 1200×630 للمقالات بلا غلاف |

كلها مولَّدة من الهوية **كما يرسمها الموقع فعلًا** (`nav/index.tsx:37–50`): Arial Black · وزن 900 · `letter-spacing: 0.25em` · `linear-gradient(135deg, #6C2BFF, #00CFFF)` مقصوصًا على النص. **لا تُعِد توليدها بقيم تقريبية.**

> 🔎 **وتحقّق الشفافية من ترويسة PNG لا بالنظر:** `colorType=6` يعني RGBA. العارض يُظهر الشفاف أبيض، فالنظر لا يفرّق بينه وبين خلفية بيضاء صلبة.

### 🖼️ أيقونة الموقع (favicon) — **كانت افتراضية القالب حتى 2026-09-13**

**بند كان مفقودًا من كل قائمة في هذا الملف.** ظلّ الموقع يشحن أيقونة قالب Medusa منذ الإطلاق — تظهر في تبويبات المتصفح **وفي نتائج Search Console**، حيث هي الشيء البصري الوحيد الذي يراه الباحث بجانب النتيجة.

**والجرد كشف أكثر من «أيقونة قديمة»:** لم يكن في المشروع إلا `public/favicon.ico` من القالب — **بلا `app/icon` ولا `apple-icon` ولا manifest**. أي أن أندرويد وiOS لم يكن لديهما أيقونة منّا إطلاقًا.

**التركيب اليوم — والاتفاقيات مؤكَّدة من HTML الصادر لا من وضع الملفات:**

| الملف | ما يُخرجه |
|---|---|
| `app/icon.png` | `<link rel="icon" sizes="192x192">` |
| `app/apple-icon.png` | `<link rel="apple-touch-icon" sizes="180x180">` |
| `app/manifest.ts` | `<link rel="manifest">` — **وأيقونتا أندرويد 192/512 لا تصلان إلا منه** |
| `public/favicon.ico` | الطلب المجرّد `/favicon.ico` الذي يرسله كل متصفح وزاحف بلا وسم |

> ⛔ **ولا تُعِد استعمال `logo-filled` أيقونةً:** فيه وردمارك `PROMPTR` تحت الحرف، ويصير لطخة عند 16px. الأيقونات نسخة مخصّصة (المربّع والحرف وحده)، **وكل حجم مرسوم أصلًا في مقاسه** لا مُصغَّرًا من واحد.
>
> ⚠️ **وPillow يخون بصمت في ICO:** `im.save(..., format='ICO', sizes=[...])` **يُصغّر مصدرًا واحدًا**، فإن كان المصدر 16px أخرج ملفًا **بصورة واحدة بلا أي خطأ**. الحاوية هنا مبنية يدويًا (ترويسة + مدخل 16 بايت لكل صورة + PNGات كما هي)، **وقُرئت بمفكّك مستقل** للتأكد من الأربع.
>
> 🔎 **والأيقونة تحوي 256×256** إضافةً إلى 16/32/48، لأن أيقونة القالب كانت تحويه — وإسقاطه كان سيكون تراجعًا خفيًا.

**مقيس على الحيّ (2026-09-13):** `/favicon.ico` ⟵ **200 · `image/x-icon` · 56,598 بايت · [16,32,48,256]**، **مختلفة بالبايت** عن أيقونة القالب (25,931). والمانيفست وأيقونتا أندرويد و`apple-icon` كلها 200.

### حساب ميسر — الاسم المعروض والشعار

**الحقل المؤثّر `public_name` لا `name`.** الأول ما يراه المشتري (الفواتير · صفحات ميسر · وغالبًا كشف حساب البطاقة)، والثاني تسمية داخلية.

**وكان `public_name` اسم المالك الشخصي** حتى 2026-09-12 — على متجر يعمل باسم Promptr. وهذا أشيع أسباب المنازعات في المتاجر الرقمية: «من خصم مني؟».

| البند | الموضع في اللوحة | الحالة |
|---|---|---|
| **الاسم العام** | تبويب **«المعلومات العامة»** ⟵ حقل **«الإسم العام»** | ✅ صار `Promptr` — **مؤكَّد من `GET /v1/account`** بالمفتاح السري، لا من الواجهة |
| **رابط موقع التاجر** | نفس التبويب | ✅ مملوء |
| **الشعار** | نفس التبويب | ✅ مرفوع — أيقونة P ظاهرة بدل الشعار الافتراضي |

> ⚠️ **ولا تتوقع ظهور الشعار في صفحة الدفع عندك:** نموذج ميسر (MPF) **مُضمَّن داخل صفحتنا** ومغلَّف بهويتنا، فلا يعرض شعارًا من جانب ميسر. قيمة الشعار في **الفواتير** وروابط الدفع المُنشأة من اللوحة.
>
> 🔎 **وقياس التغيير من API لا من اللوحة:** `name` تغيّر أولًا بينما بقي `public_name` على الاسم الشخصي — أي أن الواجهة أظهرت نجاحًا لحقل غير الحقل المؤثّر. **التحقق دائمًا بـ`GET /v1/account`.**
...
> ✅ **الجدول أدناه مقيس بالكامل (2026-09-12)** — كان استنتاجًا من المواصفة يحمل تحذيرًا «لا تستشهد به»، **وقد سقط التحذير**. قيست صفوفه الثلاثة في **WebKit (محرك سفاري) على الموقع الحي** بسلة حقيقية، والمقياس محفوظ ويُعاد تشغيله: `apps/storefront/scripts/samesite-probe.mjs`.
>
> **وما جعله قياسًا لا صدفة:** ضابط موجب قبله مباشرة يثبت أن السلة موجودة وقابلة للبلوغ في نفس المتصفح وفي نفس الدقيقة — فلا تلتبس «الكوكي محجوب» بـ«لا سلة أصلًا». والصف الأول وحده خالف الآخرَين بنفس جرّة الكوكيز.
>
> ⚠️ **ملاحظة على القياس لا ثغرة فيه:** الوصول في المقياس **نقرة رابط** من أصل آخر، وعودة ميسر الحقيقية **تحويل 302**. كلاهما تنقّل cross-site من المستوى الأعلى و`Strict` يحجب فيهما معًا — لكنهما ليسا الشيء نفسه حرفيًا.
> ⛔ **ولو أُرخي الكوكي إلى `Lax` لاختلفا:** الـLax يُرسل في تنقّل GET من المستوى الأعلى، فتصير حالة المقياس غير ممثِّلة للواقع **ويسقط CSRF عن المسار الوحيد الذي يحرّك المال**. سبب إضافي لعدم إرخائه.
...
| `MOYASAR_WEBHOOK_SECRET` | **غير مضبوط عمدًا** — ولا يُضبط قبل تسجيل webhook. غيابه يجعل `getWebhookActionAndData` **يرفض كل حدث**، وهو الاتجاه الصحيح للفشل |
...
~~3. **تحقق يدوي من إعدادات Cloudflare R2**~~ — ✅ **مُغلق 2026-09-11.** فحصه المالك بصريًا في لوحة Cloudflare على bucket `promptr-files`: **`Public Access: Disabled`** · 12 ملف PDF · 22.19 MB · **`Class A Operations = 0`**.
   > 🔒 **ويؤكده قياس من الخارج:** ملفات الـbucket الخاص **غير قابلة للوصول** عبر النطاق العام المعروف — `pub-896449c4…r2.dev/<ملف>.pdf` يردّ **404** للثلاثة المجرَّبة، بينما صورة عامة على نفس النطاق تردّ 200. الـbucketان معزولان فعلًا.
   >
   > ⛔ **والتحقق يبقى يدويًا دائمًا:** لا يوجد Cloudflare API token في بيئة العمل — المتاح مفاتيح **S3-compatible** فقط، وهي تقرأ الكائنات **ولا تقرأ إعدادات الـbucket**. فالوكيل لا يستطيع تأكيد هذا البند ولا نفيه؛ ينفيه جزئيًا من الخارج وحسب. **أي إعادة فحص مستقبلية تمرّ باللوحة.**
   >
   > 💡 **ومعرّف الحساب للرجوع إليه:** `8e6feaf55e8e2b16e30e47579b3213ac` (مشتقّ من `S3_ENDPOINT`)، والرابط المباشر `dash.cloudflare.com/8e6feaf55e8e2b16e30e47579b3213ac/r2/overview`.
~~4. **إصلاح تحقق توقيع Moyasar webhook (HMAC)**~~ — ✅ **مُغلق 2026-09-12.** المسار محصَّن و**معطَّل عمدًا**؛ التفاصيل في قسم *webhook ميسر* أدناه.

---

## 🔔 webhook ميسر — محصَّن ومعطَّل بقرار (2026-09-12)

**الحالة:** `getWebhookActionAndData` يتحقق من المُرسِل ثم **يُرجع `not_supported` دائمًا**. الثغرة مغلقة، والمسار لا يفعل شيئًا.

### ⛔ التسمية كانت خاطئة منذ فتح البند — وكادت تهدر جولة

كُتب البند «**تحقق توقيع HMAC**» طوال الوقت. **وميسر لا توقّع الحمولة إطلاقًا:** لا ترويسة توقيع، ولا تجزئة على الجسم الخام. ما ترسله حقل **`secret_token` داخل الجسم**، **يختاره التاجر** عند إنشاء الـwebhook ([مرجع ميسر](https://docs.moyasar.com/api/other/webhooks/webhook-reference/)). فالتحقق **مقارنة سرّ مشترك** ثابتة الزمن، و`rawData` **لا يُستعمل** لأن لا شيء موقَّع كي يُتحقق منه.

> **الدرس:** اسم البند في هذا الملف ليس مصدرًا. **اقرأ توثيق الطرف الآخر قبل أن تبني على التسمية** — كنت سأكتب HMAC على واجهة لا تملكه.

### المظروفان — العلّة كانت بمستويين لا بواحد

Medusa يمرّر `{ data, rawData, headers }`، وجسم ميسر نفسه مظروف ثانٍ:
`{ id, type, created_at, secret_token, account_name, live, data: {…الدفعة…} }`

فالدفعة في **`payload.data.data`**. وما كان مكتوبًا هنا («الصواب `payload.data.id`») **غير دقيق**: ذاك معرّف **الحدث**، و`payload.data.status` لا وجود له.

> ⚠️ **ونِسبة الدفعة إلى `data` استنتاج بالقياس:** ميسر تنشر مثالًا لحدث `card_auth` وحده. فالقارئ **متسامح مع الشكلين** ويسجّل `nested=yes/no` — ليحسم أول حدث حقيقي الشكلَ بالأثر. **وقد قيس الشكلان فعلًا** في اختبار الرفض أدناه.

### 🕳️ فخّ الرابط: `moyasar_moyasar` لا `moyasar`

Medusa يركّب `pp_${مقطع الرابط}`، والمعرّف المسجَّل **`pp_moyasar_moyasar`** (من جدول `payment_provider`). فالمسار الصحيح:

```
POST /hooks/payment/moyasar_moyasar     ✅
POST /hooks/payment/moyasar             ❌ ⟵ Could not resolve 'pp_moyasar'
```

> ⛔ **والخطأ فيه يفشل صامتًا من منظور المُرسِل: يردّ 200 كالصحيح تمامًا**، والعطل لا يظهر إلا في السجل. أول محاولة اختبار في هذه الجولة ضربت المقطع الخطأ، فردّت 200 ولم يُستدعَ الكود أصلًا — وكادت تُقرأ نجاحًا.

> ### ⛔ القاعدة: **الحالة الراجعة من مسار webhook في Medusa لا تُثبت شيئًا**
>
> المسار (`api/hooks/payment/[provider]/route.js`) **يردّ 200 دائمًا** ثم يُصدر حدثًا **بتأخير 5 ثوانٍ**، والمعالجة تجري في **subscriber** خارج الطلب. فالـ200 تعني «استُلم الطلب» لا «قُبل» ولا «رُفض» ولا حتى «وصل إلى المزوّد».
>
> **الإثبات من موضعين اثنين فقط:**
> 1. **سطر السجل** — `railway logs --service <backend> | grep '\[moyasar-webhook\]'`
> 2. **قاعدة البيانات** — عدّادات `payment` · `order` · `payment_session` · `moyasar_payment_claim` قبل وبعد، وبحث مباشر عن المعرّف المزوَّر.
>
> 🔁 **وهذا خامس فحص في هذا المشروع يكاد يمرّ بلا أن يفحص** (سبقه: `curl` بجرّة كوكيز · `curl` بلا محرّك SameSite · خطأ pg مختلق في التجارب · حقل إدخال محقون بيدي). **النمط واحد: الفحص يقيس ما يُنتجه الفاحص لا ما يفعله النظام.** اسأل دائمًا: ما الذي كان سيتغيّر في هذا القياس لو كان الكود معطّلًا تمامًا؟ إن كانت الإجابة «لا شيء» فالفحص لا يفحص.
>
> **6️⃣ وسادسها من نوع مختلف — تبديدٌ معيب لا قياس معيب (2026-09-12):** بحثًا عن سبب الـsoft-404 أُزيل `blog/[slug]/loading.tsx` وحده ⟵ 200، ثم `blog/loading.tsx` وحده ⟵ 200، فبدا أن **لا علاقة لأيٍّ منهما**. وإزالتهما معًا ⟵ **404**.
> ⛔ **القاعدة:** حين **يكفي أحد عاملين وحده** لإحداث الأثر، فإن **تعطيلهما بالتناوب ينفي كليهما زورًا**. لا تُبرّئ عاملًا باختبار فردي قبل أن تسأل: هل يكفي الآخر وحده؟ وإن كان الجواب نعم أو مجهولًا، **عطّلهما معًا أولًا** ثم بدّد.
>
> **7️⃣ وسابعها أُمسك قبل أن يُبلَّغ (2026-09-12):** عند رفع التصنيفات إلى `sitemap.xml` أعطى استعلامٌ بصيغة الأدمن (`*products` موسَّعًا) **8 / 6 / 3** منتجًا، بينما تعرض الصفحات **8 / 3 / 1** — فبدا أن الصفحات تُسقط منتجات. **ولا عطل:** الصفوف الزائدة **غير مرئية للستورفرنت** (قناة بيع أو حالة نشر)، و**Store API — المسار الذي تستعمله الصفحة فعلًا — يعطي 8 / 3 / 1 مطابقًا تمامًا**.
> ⛔ **القاعدة:** **عدّ الصفوف من قاعدة البيانات أو من واجهة الأدمن لا يقيس ما يراه الزائر.** حين تقارن «ما يجب أن يظهر» بـ«ما ظهر»، اجعل طرف المقارنة **نفس المسار الذي تسلكه الصفحة** — وإلا قِست نظامين مختلفين وأبلغت عن فرق بينهما كأنه عطل.
>
> **8️⃣ وثامنها مقام النسبة (2026-09-15):** أُبلغ ارتفاع زخرفة البنر بـ**67%** وهي فعلًا **78%** — لأن القاسم كان صندوق الكاروسيل (396px: الشريحة + صفّ النقاط) لا الشريحة (340px) التي يقصدها الهدف «70–80% من ارتفاع البنر». الفحص كان يعمل والرقم صحيح حسابيًا، والنتيجة مضلِّلة.
> ⛔ **القاعدة:** حين يُطلب هدف كنسبة، **اجعل القاسم هو الشيء الذي سمّاه الطالب** لا أقرب عنصر في الـDOM. وقبل تقييم أي نسبة اسأل: نسبة إلى ماذا بالضبط؟
>
> **9️⃣ وتاسعها الفاحص يقيس عيبه هو (2026-09-15):** لقطة محلية لملف HTML ظهرت بنص عربي مشوَّه، فبدا المحتوى معطوبًا — والسبب أن ملف المعاينة **بلا `<meta charset>`** حين يُفتح بـ`file://` (مضيف الـartifact يضيفه، فلا يظهر العطل هناك). وفي نفس اليوم: `textContent` لرابط السلة أعطى «السلة (1)1» فبدا العدد مكرّرًا — وهو يضمّ نصّ الشارة **المخفية بـ`display:none`**؛ `innerText` أعطى «السلة (1)» وهو المرسوم.
> ⛔ **القاعدة:** قبل الإبلاغ عن عطل ظهر في أداة قياس، افحص الأداة نفسها بحالة معلومة السلامة. و**لقياس ما يُرسم استعمل `innerText` أو `getBoundingClientRect`، لا `textContent`** الذي يضمّ المخفي.
>
> **🔟 وعاشرها كوميت يصف تعديلًا لم يدخله (2026-09-15، `0ae0cc0`):** سلسلة `python … ; git commit` — فشل التعديل (مجلد العمل انتقل إلى جذر المستودع بين نداءين فلم يجد الملف) ومضى الكوميت بالرسالة التي تصف التعديل الغائب. أُمسك من الرسالة نفسها وصُحّح في `f7032b3`.
> ⛔ **القاعدة:** اربط التعديل بالكوميت بـ`&&` لا `;` حتى يوقف الفشلُ الكوميتَ، **واقرأ `git diff --cached --stat` قبل الرسالة** لا بعدها. ومجلد العمل في هذه البيئة **يُعاد ضبطه بين النداءات** — ابدأ كل أمر بمسار مطلق أو `cd` صريح.

### ما قيس على الإنتاج (2026-09-12) — لا يُعاد

ثلاث حمولات مزوّرة إلى المسار الصحيح، وكلها تحمل `status: "paid"`:

| الحالة | الشكل | النتيجة |
|---|---|---|
| سرّ خاطئ | متداخل (`nested=yes`) | **`REJECTED`** |
| بلا حقل `secret_token` | متداخل | **`REJECTED`** |
| جسم مسطَّح | `nested=no` | **`REJECTED`** |

**وقاعدة الإنتاج بلا فرق:** `payment` 25⟵25 · `captured` 4⟵4 · `order` 25⟵25 · `claims` 1⟵1 · `sessions` 36⟵36 · `authorized` 24⟵24، وبحث عن المعرّفات المزوّرة ⟵ **صفر في الجداول الثلاثة**.

وسبع تجارب وحدة دائمة في `moyasar/__tests__/webhook.unit.spec.ts` (المجموع 24 خضراء).

### ⏳ بند مفتوح — تفعيل الـwebhook محجوب بـ`session_id` لا بالتوقيع

**التوقيع لم يكن الحاجز الحقيقي.** حتى بعد التحصين لا يستطيع هذا المسار أن يفعل شيئًا:

`processPaymentWorkflow` يبحث عن الجلسة بـ`filters: { payment_session_id: input.data?.session_id }`، و**الـmetadata التي نكتبها لا تحمل إلا `cart_id`**.

> ⛔ **والسبب بنيوي لا سهو:** الـmetadata تُكتب في المتصفح **قبل** الدفع (`moyasar-form`)، وجلسة الدفع تُنشأ في صفحة الـcallback **بعده**. فالجلسة لم تكن موجودة لحظة كتابة الـmetadata، ولا يمكن حشو `session_id` هناك.
>
> **فأي تفعيل يحتاج ربطًا بديلًا:** البحث عن الجلسة بـ`payment_session.data.moyasar_id` (وهو محفوظ فعلًا) بدل `session_id`، وغالبًا مسارًا خاصًا بنا بدل `processPaymentWorkflow`.
>
> 💡 **وفائدته الوحيدة المعروفة:** إنقاذ من دفع ثم أغلق التبويب قبل عودة الـcallback. **ولا دليل أن هذه الحالة وقعت** — فالبند مؤجَّل بقرار حتى يظهر دليل، لا لأنه صعب.
>
> ⛔ **ولا يُسجَّل webhook في لوحة ميسر قبل ذلك** — لا فائدة منه، وكل حدث سيُرفض أو يُهمَل. وحين يُسجَّل: السرّ يُختار ويُوضع في **`MOYASAR_WEBHOOK_SECRET`** على Railway **في نفس اللحظة**.
...
**`sitemap.xml` (محدَّث 2026-09-12):** **40 رابطًا** = 8 ثوابت + 18 مقالًا + 12 منتجًا + **3 تصنيفات**.

**والتصنيفات أُضيفت بعد أن انقضى شرط تأجيلها** («حتى إعادة تسميتها في Admin») — فُحص لا افتُرض: الثلاثة بأسماء عربية سليمة ومقابض نظيفة، وكلٌّ يردّ 200 بصفر تحويلات وبـcanonical إقليمي صحيح، والمقبض المجهول يردّ 404.

> ⚠️ **و`fetchCategories` يخالف `fetchProducts` في نقطة واحدة متعمَّدة: القائمة الفارغة ليست فشلًا.** صفر منتجات خلل (المتجر لم يكن فارغًا قط)، لكن **متجرًا بلا تصنيفات حالة مشروعة** — وهذه الثلاثة أُنشئت بعد الإطلاق. **لا توحّد معالجتهما.**
...
**خط Cairo** كان محمّلًا في `blog/layout.tsx` وحده حتى 2026-09-14؛ **صار في `app/layout.tsx` للموقع كله** (`47b3750`) بعد مراجعة بصرية لصفحات البيع، مع قاعدة الوراثة لأصناف `txt-*` في `globals.css` — انظر قسم *المرور البصري*.
...
**SEO:** لكل مقال `title` و`description` و`canonical` وOpen Graph وJSON-LD بمخطط `Article`. أُضيف كذلك `src/app/sitemap.ts` و`src/app/robots.ts` **الأصليان من Next** — وهما المصدر الوحيد لهما. **و`next-sitemap.js` حُذف في 2026-09-12** بعد التحقق من موته: الحزمة غير مثبّتة، ولا سكربت `postbuild` يشغّلها، ولا إشارة إليه إلا تعليقات تشرح أنه ميت.
> ⛔ **ولم يكن ميتًا فحسب بل معطوبًا لو شُغِّل:** `siteUrl` يقرأ `NEXT_PUBLIC_VERCEL_URL` (وننشر على Railway)، و`exclude` كان **جمع مصفوفتين بـ`+`** — وهو في JavaScript **تسلسل نصّي**: القيمة سلسلة واحدة `"/checkout,/account/*/[sitemap]"` لا قائمة، فما كان ليستثني شيئًا مما قصده.
> **ولا تُعِد `next-sitemap`:** مولّدان لنفس الملفين هما شكل العطل الذي نتجنّبه.

**`og:image` — أُصلح 2026-09-12 بعد أن كان غائبًا عن كل مقال.**

القياس قبل الإصلاح: **17 مقالًا من 17 بلا `cover`**، والصفحة الحيّة تُصدر `og:title` و`og:description` و`og:url` و`og:site_name` و`og:locale` و`og:type` — **ولا `og:image` إطلاقًا**. أي أن كل مشاركة على واتساب أو إكس كانت رابطًا أصمّ.

> ⚠️ **والسبب ليس نقص ملف:** `app/opengraph-image.jpg` الجذري موجود، لكنه **لا يُورَّث** حين تُعرّف الصفحة كائن `openGraph` خاصًا بها. فوجود صورة جذرية لا يعني أن الصفحات ترثها.

**اليوم:** بطاقة افتراضية `public/images/brand/og-default.png` (1200×630) تُستعمل حين لا `cover`، و`twitter:card` يرتقي إلى `summary_large_image`. وأي مقال يضيف `cover` **يتقدّم عليها تلقائيًا**.

> ⛔ **ولا تُمرَّر البطاقة إلى JSON-LD.** `Article.image` في schema.org يعني **صورة تُمثّل المقال**، والبطاقة شعار لا يمثّل شيئًا. وسم `og` غرضه بصري والبيانات المنظّمة غرضها وصفي — فلا يُملأ حقل وصفي بقيمة زخرفية. نفس مبدأ رفض `aggregateRating` الملفَّق.
...
**✅ مُغلق (2026-09-12): slug غير موجود يردّ 404 حقيقيًا.** كان يردّ **200** مع هيكل الموقع — أي soft-404، وGoogle تُبقي تلك الصفحات كصفحات رقيقة مكرّرة بدل إسقاطها.

**والسبب لم يكن ما بدا:** المسار كان يفعل كل ما يُفترض أن يكفي — `force-dynamic` و`notFound()` في `generateMetadata` — ومسار المنتجات يفعل الشيء نفسه ويردّ 404. **السبب ملفّا `loading.tsx`**: أيٌّ منهما يفتح حدّ تعليق، فيُبثّ الهيكل و**الحالة 200 مُلتزَمة** قبل أن يصل `notFound()`.

> ⛔ **فلا تُعِد `loading.tsx` إلى `blog/` ولا إلى `blog/[slug]/`** — عودة أيٍّ منهما تُعيد الـsoft-404 صامتًا.
>
> **وبديلٌ جُرِّب وقيس وسقط:** `generateStaticParams` + `dynamicParams = false` (رفض المسار غير المعروف على مستوى التوجيه مع إبقاء الهياكل) ⟵ **ما زال 200**. فالمفاضلة حقيقية لا مفترضة، وهذا يعكس القرار السابق («لا نضحّي بالهياكل») **ببيّنة لم تكن متاحة حين اتُّخذ**.
>
> **وما فُقد محدود:** الهيكل يظهر عند **التحميل الصلب** وحده؛ التنقّل الداخلي يُبقي الصفحة السابقة مرسومة فلا وميض.

### ⏳ بند مفتوح — صفحة 404 عارية

`app/[countryCode]/(main)/not-found.tsx` صفحة بأنماط سطرية (`textAlign` و`padding`) لا تشبه الموقع إطلاقًا: بلا خط الموقع ولا ألوانه ولا رأسه. وهي وجهة كل 404 — والآن تستقبل مسارات المدونة أيضًا. **لم يُنفَّذ، ومسجَّل بقرار.**
...
- ~~**`modules/home/components/featured-products/` صار يتيمًا (2026-07-31)**~~ — ✅ **عاد للاستعمال (2026-09-15، `4429bcc`):** `product-rail` يستلم المنتجات من `page.tsx` عبر `lib/data/home.ts` بدل الاستعلام بالمجموعات، وحُذف غلاف المجموعات `featured-products/index.tsx`. *(التاريخ: كان يرسم شريطًا لكل Collection وأُزيل استدعاؤه في 2026-07-31 مع عمود «المجموعات» في الفوتر.)* مسار `/collections/[handle]` و`lib/data/collections.ts` باقيان كما هما، فلم يُحذف أي Collection من Medusa.
  **السبب:** `/store/collections` على الباكند الحي يُرجع `count: 0` (لا مجموعات أصلًا)، بينما كانت الواجهة تعرض شريط «قسم AI» بصورة مفقودة ورابط «عرض الكل» يعطي 404 لأن `getCollectionByHandle` ينفّذ `notFound()`. القسم كان مكررًا مفهوميًا لبطاقة «أدوات الذكاء الاصطناعي» ضمن التصنيفات الثلاثة في `collection-cards/index.tsx` (وهي **Categories** لا Collections). **ومنذ `d1226c1` (2026-09-15) لم تعد قائمة ثابتة:** التصنيفات وأغلفتها وعددها تُقرأ من Store API عبر `lib/data/home.ts`؛ الاسم الإنجليزي وحده خريطة بالمقبض داخل المكوّن.
...
- ⏳ **بند مفتوح — الإبطال الفوري لكاش الكتالوج (بديل الـ60 ثانية).** أُغلق العرَض في 2026-09-11 بمهلة `CATALOG_REVALIDATE_SECONDS = 60` في `lib/data/cookies.ts` (تُطبَّق على `products` · `collections` · `categories` · `regions`) — **وهي سقف لا حلّ**: تعديل من Admin يظهر خلال دقيقة، لا فورًا.
  **الحل الجذري يحتاج ثلاثة أجزاء، ولا يعمل بأقل منها:**
  1. **مسار `revalidate` في الستورفرنت** (`app/api/revalidate/route.ts`) يستدعي `revalidateTag`.
  2. **سرّ مشترك** بين الباك إند والستورفرنت يحرس المسار — وإلا صار إبطال الكاش متاحًا لأي أحد.
  3. **وسم ثابت مستقل عن الكوكي** — وهذا هو الجزء الذي يسقط النسيان: الوسم الحالي `products-${cacheId}` مشتق من كوكي الزائر، **فالباك إند لا يعرفه ولا يستطيع تخمينه**، وعناصر الكاش التي أُنشئت بلا كوكي **بلا وسم أصلًا**. أي subscriber يُبنى على الوسم الحالي **لن يُبطِل شيئًا**.
  4. ثم subscriber في الباك إند على `product.created` / `product.updated` (وما يماثلها للتصنيفات) ينادي المسار.
  > ⚠️ **لا تبدأ من الـsubscriber.** ابدأ بالوسم الثابت، وإلا بنيت مسارًا يعمل ولا يُغيّر شيئًا.

- 🧵 **خيط مشتبه به — ولا علاقة له بالسعر القديم (فُصل بقياس 2026-09-11): عمرا الكوكيين مختلفان.** `_medusa_cart_id` عمره **7 أيام** (`lib/data/cookies.ts:77`) بينما `_medusa_cache_id` عمره **24 ساعة** (`middleware.ts:156` و`187`).
  ...
  **المشكلة المرشَّحة:** صفحة المنتج تعرض أحيانًا **سعرًا قديمًا بعد تعديله من لوحة Medusa**. الشكل يطابق: السعر يُقرأ عبر نفس مسار `force-cache` الموصوف في البند التالي، وإبطاله يعتمد على وسم قد يكون فارغًا أو منتميًا لفضاء منقرض.
  > ⛔ **وقد قِيس الربط ونُفي (2026-09-11):** سبب السعر القديم كان **غياب `revalidate`** لا عمر الكوكي. الدليل: وسم `products` لا يُبطَل إلا من موضعين (`locale-actions.ts:29` و`cart.ts:646`)، وكلاهما يُطلقه **زائر** لا الأدمن — فمهما كان عمر الكوكي، **لا يوجد حدث يُبطِل عند تعديل السعر**. توحيد العمرين ما كان ليُصلح شيئًا من ذلك العرَض.
  >
  > **ويبقى الخيط قائمًا لعرَض آخر:** زائر انقضى كوكيه ثم بدّل اللغة يُبطِل وسمًا **لا ينتمي إليه أي عنصر كاش**، بينما عنصره الفعلي يحمل وسمًا منقرضًا (أو لا وسم له). أي إبطال ضائع، لا بيانات فاسدة. **ولا يُعالَج إلا ضمن «الوسم الثابت» في البند أعلاه** — توحيد العمرين وحده يبقى بلا فائدة مقيسة.
  >
  > ⚠️ **خيط لا تشخيص.** لم يُقس أثره، ولم يُربط بعرَض قائم بدليل. **لا توحّد العمرين كإصلاح قبل القياس** — قد يكون السبب الحقيقي هو `force-cache` بلا `revalidate` في البند التالي، وتوحيد العمرين حينها يغيّر سلوك الكاش بلا أن يحلّ شيئًا. اقتُطع عمدًا من كوميت إصلاح 404 صفحة الدفع (2026-09-02) لأنه مشكلة منفصلة.
