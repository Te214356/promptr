...
## 📌 نقطة الاستئناف — 2026-09-11 (اقرأها أولًا)

> # 🎯 الأولوية القادمة **ليست تقنية**: جلب أول زوار حقيقيين.
>
> المتجر جاهز ولم يبع لعميل حقيقي بعد. الدفع يعمل، والحارسان يحرسان، والتسليم يصل، والنصوص تُقرأ، وشارات الثقة حقيقية، والسيو صار نظيفًا — **كل عذر تقني أُزيل ومقيس**. فلا تبدأ الجلسة القادمة ببند تقني إلا إن طلبه المالك صراحةً.
>
> وحجم الفجوة بالأرقام: **17 مقالًا منشورًا · 5 نقرات من بحث Google في 28 يومًا · صفر بيع لعميل**. وأقرب ما وصل المتجر إلى عملاء حقيقيين ثلاث محاولات شراء فاشلة في 28 أغسطس. الأدوات في `.claude/agents/` (بحث · كتابة · يوتيوب · استشارة).

> هذه الكتلة تَجُبّ «نقطة الاستئناف» القديمة المؤرَّخة 2026-08-08 في وسط الملف.

### ✅ مُغلق نهائيًا — **لا يُعاد فحصه**

| البند | الدليل |
|---|---|
| **مسار الشراء من طرف إلى طرف** | شراء بشري حقيقي بمفاتيح حيّة: الدفع ⟵ الحارسان ⟵ الطلب 25 ⟵ البريد ⟵ رابط التحميل. **ثلاثة مصادر متطابقة** وجدول زمني |
| **حارسا الدفع** (الربط والاستهلاك) | مقيسان على شراء حقيقي: ميسر أعاد `metadata.cart_id`، والفهرس الفريد قَبِل مطالبة واحدة |
| **البريد ورابط التحميل** | خرج من `orders@promptrsa.com` وفتح الملف |
| **تباين WCAG AA** | مسح حيّ بمحرك سفاري: **139 راسبًا ⟵ 16**، وكلها زر واتساب المستثنى |
| **شعارات الدفع** | الأربعة رسمية من مصادرها، بلا تخفيف، ونسبها محفوظة |
| **canonical** | سبع صفحات تعلن الصيغة الإقليمية — ومعه أُغلق ازدواج `/` و`/sa` |
| **كاش الكتالوج** | `revalidate: 60` على المنتجات والمجموعات والتصنيفات والمناطق. مقيس بتعديل سعر حقيقي: **59 ثانية** ذهابًا و**11** إيابًا |
| **الروابط الداخلية** | 56 رابطًا بُودئت بالمنطقة عند العرض ⟵ **صفر مجرّد**، وتحويلة موفَّرة لكل رابط. وملفات الجذر (`ads.txt` · التحقق) سليمة مقيسة |
| **TLS على `api.promptrsa.com`** | `CN=api.promptrsa.com` من Let's Encrypt · **200 في 0.46s** بعد 80 يومًا عالقًا |
| **Cloudflare R2** | `promptr-files` ⟵ `Public Access: Disabled` (فحص بصري) · والـbucketان معزولان مقيسًا من الخارج |
| **كود الخصم الاختباري** | `TEST-JSB4…` ⟵ `inactive` بعد `used: 1/1` |
| **خلفية `#080810`** | قُيست ثلاثة بدائل: **صفر صف يُحل**. بقيت بقرار — لا تُعِد الاقتراح بلا سبب جديد |

### ⏳ المفتوح — أربعة عشر بندًا

**أمن (الأثقل):**
~~1. `أ1` توقيع webhook~~ — ✅ **مُغلق 2026-09-12.** محصَّن بسرّ مشترك (**لا HMAC** — ميسر لا توقّع) ومعطَّل بقرار. ثلاث حمولات مزوّرة رُفضت على الإنتاج، والقاعدة بلا فرق.
1. **تفعيل الـwebhook محجوب بـ`session_id` المفقود** — حاجز بنيوي لا أمني، وفائدته غير مثبتة. مؤجَّل بقرار — انظر قسم *webhook ميسر*.
2. `أ3` مراجعة أمنية بشرية — لم تجرِ، والدفع حيّ.
3. `أ4` `Content-Security-Policy` — يُبدأ بـ`Report-Only`.

**أثر على البيع:**
4. **الإبطال الفوري لكاش الكتالوج** (بديل الـ60 ثانية) — مسار + سرّ + **وسم ثابت**. ⛔ ابدأ بالوسم لا بالـsubscriber.
~~5. `ب3` آلية استرجاع السلة · 6. `ب4` جدول `SameSite`~~ — ✅ **مُغلقان 2026-09-12 بقياس في WebKit على الحيّ.** الحذف كان صحيحًا، وصفوف الجدول الثلاثة صارت مقيسة. المقياس محفوظ: `apps/storefront/scripts/samesite-probe.mjs`.
5. **الفشل داخل نموذج MPF غير مغطّى** — بطاقة مرفوضة بلا 3DS تُعالَج في الصفحة بلا تنقّل، فلا يُقاس بحمولة مصاغة. **يحتاج دفعة حقيقية مرفوضة.** مؤجَّل بقرار.
7. `ب5` `requires_shipping: false` صريحًا · 8. `ب2` عمرا الكوكيين (خيط لعرَض آخر، لا للسعر القديم)

**سيو:** ~~`ج4` soft-404~~ · ~~`ج5` التصنيفات~~ · ~~`ج6` `next-sitemap.js`~~ — ✅ **الثلاثة مُغلقة 2026-09-12 ومقيسة على الحيّ** · 9. **صفحة 404 عارية** لا تشبه الموقع

**ديون تقنية:** 12. `د3` `/code-review ultra` (يُطلقه المالك) · `د4` ترقيات npm · `د5` عدّادات الذاكرة ⟵ Redis · `د6` `app/error.tsx` الجذري · `د7` اليتيمان · `د8` الجزيرتان الفاتحتان · `د9` واتساب (مستثنى بقرار) · `د10` `sku`

**محتوى وتسويق:** 10. `هـ1` AdSense (معلومات الدفع · صفر وحدة إعلانية) · 11. `هـ2` مقالات جديدة · ~~`هـ3` شعار ميسر~~ ✅ **مُغلق 2026-09-12** — الشعار مرفوع، و`public_name` صار `Promptr` مؤكَّدًا من API
