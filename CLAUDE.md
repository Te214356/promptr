...
## استرجاع السلة بعد العودة من ميسر — موقَّع، وفاشل مغلقًا (2026-09-03)

`_medusa_cart_id` كوكي **`SameSite=Strict`**، فالمتصفح **لا يرسله** في تنقّل قادم من موقع آخر — وهو بالضبط شكل العودة من صفحة 3-D Secure. فكانت صفحة الدفع تُرجع **404** لمن يعود أو يحدّث الصفحة.

**الحل:** `app/api/checkout-session/route.ts` — يعيد كتابة الكوكي ثم يحوّل إلى الدفع. Route Handler لا صفحة، لأن `cookies().set()` يرمي داخل رندر مكوّن خادم (نفس سبب وجود `/api/order-complete`).

> ⛔ **لا تُرخِ الكوكي إلى `Lax` كحلّ.** ذلك يعالج العرَض بإضعاف حماية CSRF على المسار الوحيد الذي يحرّك المال. الـhandler يعطي النتيجة نفسها بلا مساس بها.

### التوقيع — ولماذا «السلة القائمة تفوز» وحدها لم تكن كافية

حارس «لا نستبدل سلة قائمة» يمنع **سرقة** سلة، لكن الضحية في السيناريو الحقيقي **لا تملك سلة أصلًا**: يبني المهاجم سلة بإيميله ومنتجاته، ويرسل `‎?cart_id=…‎` لمن جلسته فارغة، فيدفع هو ويستلم المهاجم بريد التأكيد **وروابط التحميل الموقّعة** (٤٨ ساعة).

فالمعرّف لا يُقبل إلا بتوقيع **HMAC-SHA256** سكّه هذا الخادم: `lib/util/checkout-handoff.ts`، بصيغة `<انتهاء>.<توقيع>`، **والانتهاء داخل الحمولة الموقَّعة** فلا يُمدَّد بتحرير الرمز. العمر **15 دقيقة** — تكفي جولة 3DS وتضع سقفًا لقيمة أي رابط مسرَّب.

**موضع السكّ ليس تفصيلًا:** `checkout-form/index.tsx` (مكوّن خادم)، أي **اللحظة الوحيدة التي نعرف فيها أن السلة سلة المشتري** لأنها قُرئت من كوكيه هو، قبل أن يغادر إلى ميسر. الرمز يسافر في `callback_url` ويعود.

> ⛔ **لا تسكّ التوقيع في صفحة الـcallback من `cart_id` القادم في الرابط.** ذلك يصدر توقيعًا صالحًا لأي معرّف يضعه المهاجم — أي إعادة فتح الثغرة بالتوقيع نفسه الذي يُفترض أن يغلقها.

| المتغير | `CHECKOUT_SESSION_SECRET` |
|---|---|
| المكان | **متغير خدمة على Railway (الستورفرنت)** + `.env.template` |
| **لا** يحتاج `turbo.json` | يُقرأ **وقت التشغيل** فقط، ولا يُخبز في مخرَج البناء — فلا يدخل قائمة `env` في `turbo.json` (تلك للبناء) |
| الشكل | 16 محرفًا فأكثر، `openssl rand -hex 32` |
| ⛔ | **لا تسبقه بـ`NEXT_PUBLIC_`** — يشحن مفتاح التوقيع إلى المتصفح فيصير كل رمز قابلًا للتزوير |

**fail-closed بقرار:** بلا سرّ، **لا يُسكّ رمز ولا يُقبل استرجاع**. أثره أن العائد من ميسر يجد نفسه في صفحة السلة بدل خطوة الدفع — مزعج، لكنه أفضل من 404 الحالي، **وأفضل بكثير** من قبول معرّف غير موقّع. قِيس: بلا سرّ ⟵ صفر `&t=` في `callback_url`، والاسترجاع مرفوض.
...
> ⚠️ **ثلاثة من هذه الخمسة كانت تعرض «إعادة المحاولة» بعد الخصم** — أي دعوة صريحة لخصم ثانٍ. أخطرها فرع `initiatePaymentSession`: كان رابطه **مكسورًا** (ينتهي بـ404 لضياع الكوكي)، وإصلاح الـ404 **جعله يعمل** — فتحوّل رابط خطر معطّل إلى رابط خطر صالح. الدرس: عند إصلاح مسار مكسور، افحص ما الذي كان الكسر يحجبه.
>
...
في `lib/util/payment-errors.ts` الترتيب هو المنطق. `network` (النقل) يُختبر **قبل** `card_declined`، لأن نمط الأخير يحوي `\brefus` و`issuer` — فكانت `Connection refused` تُصنَّف **رفض بطاقة**، فيُقال للمشتري «جرّب بطاقة أخرى» بينما البوابة غير قابلة للوصول أصلًا؛ يجرّب بطاقات بلا داعٍ ويُلوَّث كل تشخيص لاحق. حدّ `\b` كان يحمي من `ECONNREFUSED` الملتصقة وحدها.
...
### التجارب الخمس — أعِدها كاملة عند أي تعديل على هذا المسار

بلا جرّة كوكيز (قاعدة الزاحف أعلاه تنطبق هنا أيضًا: الجرّة تخفي عطل الكوكي المفقود):
...
| 4 | `cart_id` + توقيع صحيح، بلا كوكي | 307 ⟵ handler ⟵ 307 + `Set-Cookie` ⟵ **200** |
| **5** | **باك إند ساقط + بلا كوكي** | ⟵ `?notice=cart_unavailable`، **وليس `cart_expired`** |

> ⚠️ **التجربة 5 موجودة لأن التجربة 3 أخفتها.** كانت 3 تمرّ (500 صحيح) بينما الحالة «بلا كوكي + انقطاع» تنتهي بـ«انتهت جلسة السلة» — كذبة: `retrieveCart` يُرجع `null` **قبل أي نداء شبكة** حين لا يوجد معرّف (`cart.ts`: `if (!id) return null`)، فـ`throwOnFailure` لا يُنقذ. الحلّ في الـhandler: **وجهة الانقطاع تُختار بحسب وجود الكوكي** — بكوكي ⟵ الدفع (حدّ الخطأ)، بلا كوكي ⟵ `cart_unavailable`.
>
> **وحدّ معروف:** أثناء انقطاع **كامل** تُرجع صفحة السلة 500 على أي حال، لأن تخطيط `(main)` نفسه يقرأ من الباك إند — سبب سابق لهذا الإصلاح ولم يُمَس. المُنجَز هنا أن **الرسالة لم تعد كاذبة**، لا أن الصفحة صارت تعمل بلا باك إند.

**اختبارات الزرع التي يجب أن تبقى مرفوضة:** بلا توقيع · توقيع مزوّر · توقيع منتهٍ · **توقيع صحيح لسلة أخرى**. الأربعة تنتهي بصفحة السلة، ولا يُكتب كوكي.
