...
const T = {
  ar: {
    cart: (n: number) => `السلة (${n})`,
    cartTitle: "السلة",
    quantity: (n: number) => `الكمية: ${n}`,
    remove: "حذف",
    subtotal: "المجموع",
    exclTax: "(قبل الضريبة)",
    ...
    <div className="h-full z-50" onMouseEnter={openAndCancel} onMouseLeave={close}>
      <Popover className="relative h-full">
        ...
        >
          <PopoverPanel
            static
            className="hidden small:block absolute top-[calc(100%+1px)] right-0 bg-[#0d0d1f] border border-white/10 w-[420px] text-white rounded-xl"
            data-testid="nav-cart-dropdown"
          >
            <div className="p-4 flex items-center justify-center border-b border-white/5">
              <h3 className="text-base font-semibold text-white">{t.cartTitle}</h3>
            </div>
            {cartState && cartState.items?.length ? (
              <>
                <div className="overflow-y-scroll max-h-[402px] px-4 grid grid-cols-1 gap-y-8 no-scrollbar p-px">
                  {cartState.items
                    ...
                    .map((item) => (
                      <div className="grid grid-cols-[122px_1fr] gap-x-4 py-4" key={item.id} data-testid="cart-item">
                          ...
                          <div className="flex flex-col flex-1">
                            <div className="flex items-start justify-between">
                              <div className="flex flex-col overflow-ellipsis whitespace-nowrap mr-4 w-[180px]">
                                ...
                                />
                                <span className="text-white/40 text-xs" data-testid="cart-item-quantity" data-value={item.quantity}>
                ...
                <div className="p-4 flex flex-col gap-y-4 border-t border-white/5">
                  <div className="flex items-center justify-between">
                    <span className="text-white/60 text-sm font-semibold">
                      {t.subtotal}{" "}
                      <span className="font-normal text-white/30">{t.exclTax}</span>
                    </span>
                    <span className="text-white font-semibold" data-testid="cart-subtotal" data-value={subtotal}>
              ...
              <div className="flex py-16 flex-col gap-y-4 items-center justify-center">
                <div className="bg-white/5 border border-white/10 flex items-center justify-center w-8 h-8 rounded-full text-white/50 text-sm">
                  <span>0</span>
                </div>
                <span className="text-white/40 text-sm">{t.emptyMsg}</span>
