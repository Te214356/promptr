...
const ShippingDetails = ({ order }: ShippingDetailsProps) => {
  ...
  const isDigital = !order.shipping_methods?.length

  return (
    <div>
      <h2 className="text-xl font-semibold text-white mb-4">
        {isAR ? (isDigital ? "معلومات التواصل" : "التوصيل") : (isDigital ? "Contact" : "Delivery")}
      </h2>

      {isDigital ? (
        // Digital product: show only email
        <div data-testid="shipping-contact-summary">
          <span className="text-sm font-semibold text-white/70 mb-1 block">
            {isAR ? "البريد الإلكتروني" : "Email"}
          </span>
          <span className="text-sm text-white/50">{order.email}</span>
        </div>
      ) : (
        // Physical product: show full shipping details
